/**
 * Shell breadcrumb — primary org/agent navigation: `org › agent`.
 *
 ...
 *
 * Renders in the desktop sidebar header and the mobile toolbar.
 */
    ...
    return (
      <AgentScopePicker
        ...
        trigger={
          <button
            type="button"
            aria-label={`Switch agent — ${title}`}
            className="wco-no-drag flex size-8 items-center justify-center rounded-md transition-colors hover:bg-accent/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
          >
            <AgentAvatar
...
export function ShellBreadcrumb({
  ...
  const compact = placement === "sidebar" && sidebarState === "collapsed";
  ...
  return (
    <Breadcrumb className={cn("wco-no-drag min-w-0", compact && "w-full")}>
      <BreadcrumbList
        className={cn(
          "flex-nowrap gap-1.5 sm:gap-1.5",
