...
const GIT_QUERY_KEYS = {
  branch: (connId: string) => ["git", "branch", connId] as const,
  branchList: (connId: string) => ["git", "branchList", connId] as const,
  status: (connId: string) => ["git", "status", connId] as const,
  diff: (connId: string) => ["git", "diff", connId] as const,
  log: (connId: string) => ["git", "log", connId] as const,
};
...
/**
 * Hook to get the model connection's MCP client for AI features.
 */
function useModelClient(): {
  ...
  const firstModelConn = modelConnections[0];

  const modelClient = useMCPClientOptional({
    connectionId: firstModelConn?.id,
    orgId: org.id,
  });

  // We'll pick the model lazily — we need the model list
  // For now, return client + null modelId (resolved at call time)
  return { modelClient, modelId: null };
}
...
export function GitPanel({
  ...
  const watcher = useConnectionWatch(connectionUrl, refreshAll);
...
/**
 * Hook to get the change count for the topbar indicator.
 * Uses SSE file-watch to invalidate reactively instead of polling.
 */
export function useGitChangeCount(
  ...
  useConnectionWatch(connectionUrl, () => {
    if (connectionId) {
      queryClient.invalidateQueries({
        queryKey: GIT_QUERY_KEYS.status(connectionId),
      });
  ...
  const { data: files = [] } = useQuery({
    queryKey: connectionId
      ? GIT_QUERY_KEYS.status(connectionId)
      : ["git", "status", "skip"],
    queryFn: () => (client ? gitStatus(client) : []),
