...
function buildPreviewUrl(
  ...
  if (target.kind === "saved") {
    return buildGlobalSectionPreviewUrl(
      previewUrl,
      livePageResolveType,
      target.blockKey,
    );
  }
  return buildInlineSectionPreviewUrl(
    previewUrl,
    livePageResolveType,
    target.resolveType,
    target.data,
...
/**
 * Collapsible live preview of a single CMS section, rendered in a sandboxed
 * iframe pointed at the sandbox dev server's `/live/previews/<page>` endpoint.
 * Bumping `reloadKey` recomputes the URL (fresh cache-bust) so the iframe
 * reloads after an autosave or while editing. When collapsed, a thin rail with
 * an eye icon reopens it.
 */
export function SectionPreviewPane({
  ...
  if (prevDepKey !== depKey) {
    ...
    setSrc(buildPreviewUrl(previewUrl, livePageResolveType, target, theme));
  }
