    ...
    >
      {required && (
        <span className="absolute -top-3 right-4 rounded-full border border-input bg-background px-3 py-1 text-xs font-medium text-foreground">
          {t("reportsOnboarding.companionCard.required")}
        </span>
      )}
...
export function ConnectAction({
  ...
  return (
    <Button
      ...
      onClick={onConnect}
      aria-label={t("reportsOnboarding.companionCard.connectAriaLabel", {
        title,
      })}
    >
      {connecting ? (
        ...
        t("reportsOnboarding.companionCard.connect")
      )}
    </Button>
  );
}
...
export function ConfigureAction({
  ...
  return (
    <Button
      ...
      disabled={disabled}
      onClick={onConfigure}
      aria-label={t("reportsOnboarding.companionCard.configureAriaLabel", {
        title,
      })}
    >
      <Settings01 size={16} />
      {t("reportsOnboarding.companionCard.finishSetup")}
    </Button>
  );
}
    ...
    <div className="flex items-center gap-1">
      <span className="flex items-center gap-1.5 text-sm text-muted-foreground">
        ...
        {detail ? (
          ...
          t("reportsOnboarding.companionCard.connected")
        )}
      </span>
      {controls}
    </div>
