...
import { reportNavTarget } from "@/hooks/use-reports-diagnostic";
...
/**
 * Blocking reports-onboarding connections step, rendered as a modal OVER the
 * org (the report page behind it stays mounted and blurred) instead of as a
 * standalone `/reports-onboarding` screen. Non-dismissable: no close button, no
 ...
 * report.
 */
export function ReportsConnectModal({ siteUrl }: { siteUrl?: string }) {
   ...
   * and reveals the report. Target end state: the Reports report app
  ...
  const goToReport = () => {
    ...
    const target = reportNavTarget(org, WellKnownOrgMCPId.REPORTS(org.id));
    navigate({
      ...target,
      ...
      <DialogPortal>
        ...
        >
          <DialogTitle className="sr-only">
            {t("routes.reportsOnboarding.connectModal.dialogTitle")}
          </DialogTitle>
          <ErrorBoundary
            fallback={() => (
              <div className="grid gap-4 p-6">
                ...
                <Button
                  ...
                  onClick={goToReport}
                >
                  {t("routes.reportsOnboarding.connectModal.continueButton")}
                  <ArrowRight size={18} />
                </Button>
              </div>
            )}
...
function ReportsConnectModalContent({
  siteUrlFromUrl,
  onComplete,
}: {
  ...
  const metadataSiteUrl =
    typeof item?.metadata?.siteUrl === "string"
      ...
      : undefined;
  // Prefer the site from the URL (same context as /reports-onboarding?siteUrl=…);
  ...
  const openReport = async () => {
    ...
    if (normalized?.ok) {
      try {
        ...
        // Reports side (not_upgraded). Reconcile by re-running setup
        ...
        if (!runResult.triggered) {
          ...
          setRunError(
            t("routes.reportsOnboarding.connectModal.couldNotGenerateReport"),
          );
          return;
        }
      } catch (err) {
          ...
          <div className="flex gap-3">
              ...
              <ConnectFooterButton
                ...
                label={
                  runMutation.isPending
                    ? t("routes.reportsOnboarding.connectModal.openingReport")
                    : t("routes.reportsOnboarding.connectModal.viewDiagnostic")
                }
            ...
            {!ready ? (
              ...
              <Button
                ...
                variant="secondary"
                size="lg"
                className="h-auto min-h-12 shrink-0 classic:rounded-lg py-3 text-base font-medium leading-tight"
                disabled={runMutation.isPending}
                onClick={() => void openReport()}
              >
                {t("routes.reportsOnboarding.connectModal.skip")}
              </Button>
            ) : null}
          </div>
        </>
