...
import { useReportsCompanions, useReportsSiteUrl } from "./use-companions.ts";
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent aria-describedby={undefined} className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {t("reportsOnboarding.connectSourceDialog.title")}
          </DialogTitle>
        </DialogHeader>
        {children}
      </DialogContent>
    ...
    >
      <Suspense
        fallback={
          <StatusDialog onClose={onClose}>
            <SpinnerRow
              label={t("reportsOnboarding.connectSourceDialog.loading")}
            />
          </StatusDialog>
        }
      >
...
function ConnectSourceDialogContent({
  ...
  const { cards } = useReportsCompanions({
    selfClient,
    org,
    cdConnectionId,
    siteUrl,
    ...
    >
      <DialogContent aria-describedby={undefined} className="sm:max-w-lg">
        ...
        {error ? (
          ...
          <SpinnerRow
            label={t("reportsOnboarding.connectSourceDialog.connecting", {
              title: card.title,
            })}
          />
        )}
