...
export interface CompanionCardModel {
   ...
   *  or "sa" (shared service-account verified binding in reports).
 ...
 *  (e.g. "google-analytics"), from the reports status endpoint. */
 ...
 * `cdConfig` is the Reports connection's configuration_state (where
...
/**
 * Static binding requirements for the four Reports companions — a
 ...
 * with reports's CONNECTION_PROVIDERS (vtex/ga4/gsc/github); if the
...
const CONFIG_FIELD_LABEL_KEYS: Record<string, TranslationKey> = {
  accountName: "reportsOnboarding.companionCard.configField.accountName",
  appKey: "reportsOnboarding.companionCard.configField.appKey",
  appToken: "reportsOnboarding.companionCard.configField.appToken",
  propertyId: "reportsOnboarding.companionCard.configField.propertyId",
  siteUrl: "reportsOnboarding.companionCard.configField.siteUrl",
};
...
export function buildCompanionCards(args: {
  ...
  /** Providers connected via the shared-SA lane (from reports). */
  ...
  for (const req of args.requirements) {
    ...
    // binding reported by reports. Mirrors run-time resolution.
