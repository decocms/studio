 ...
 * copy, the id to paste. The id goes to REPORTS_BIND, which verifies
...
export function SaBindingForm({
  ...
  >(null);
  const host = siteHost || t("reportsOnboarding.saBinding.sampleDomain");
  ...
  const schema = z.object({
    resourceId: z
      ...
      .min(
        1,
        t("reportsOnboarding.saBindingForm.resourceIdRequired", {
  ...
  const mutation = useMutation({
    mutationFn: async (values: SaFormValues): Promise<SaBindResult> => {
      if (!siteUrl) {
        throw new Error(
          t("reportsOnboarding.saBindingForm.storeUrlUnavailable"),
        );
      }
    ...
    onSuccess: async (res) => {
      if (res.ok) {
        setRemediation(null);
        toast.success(
          t("reportsOnboarding.saBindingForm.connectedSuccess", {
            label: copy.label,
          }),
        );
        if (siteUrl) {
          await queryClient.invalidateQueries({
            queryKey: KEYS.reportsConnectionStatus(org.id, siteUrl),
          });
        }
        onDone();
        return;
  ...
  const copyEmail = () => {
    ...
    toast.success(t("reportsOnboarding.saBindingForm.emailCopied"));
  };
      ...
      return (
        <div className="flex w-full items-center gap-2 rounded-lg border border-input bg-muted/40 p-2">
          ...
          <Button
            ...
            size="icon-sm"
            onClick={copyEmail}
            aria-label={t("reportsOnboarding.saBindingForm.copyEmailLabel")}
          >
            <Copy01 size={16} />
          </Button>
        </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {mutation.isError && (
        <p role="alert" className="text-sm text-destructive">
          {mutation.error instanceof Error
            ...
            : t("reportsOnboarding.saBindingForm.bindError")}
        </p>
      )}

      {onOAuthInstead && (
        <p className="text-sm leading-5 text-muted-foreground">
          {t("reportsOnboarding.saBinding.oauthNote")}{" "}
          <button
            type="button"
          ...
          >
            {t("reportsOnboarding.saBindingForm.googleLoginAlternative")}
          </button>
        </p>
      )}
      ...
      <DialogFooter className="gap-2">
        <Button
          ...
          disabled={isPending}
        >
          {t("reportsOnboarding.saBindingForm.cancel")}
        </Button>
        <Button type="submit" disabled={isPending}>
          {isPending
            ? t("reportsOnboarding.saBindingForm.verifying")
            : t("reportsOnboarding.saBindingForm.bind")}
        </Button>
      </DialogFooter>
    </form>
  );
