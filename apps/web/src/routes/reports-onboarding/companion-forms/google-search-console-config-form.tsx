...
const makeSchema = (t: TFunction) =>
  z.object({
    siteUrl: z
      ...
      .min(
        1,
        t("reportsOnboarding.googleSearchConsoleConfigForm.siteRequired"),
      ),
  });
...
export function GoogleSearchConsoleConfigForm({
  ...
  const sitesQuery = useQuery({
    queryKey: KEYS.reportsCompanionGscSites(org.id, connectionId),
    queryFn: async () => {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-destructive">
          {t("reportsOnboarding.googleSearchConsoleConfigForm.loadSitesError")}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">
          {t("reportsOnboarding.googleSearchConsoleConfigForm.noSitesFound")}
        </p>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {error && (
        <p role="alert" className="text-sm text-destructive">
          {error instanceof Error
            ? error.message
            : t("reportsOnboarding.googleSearchConsoleConfigForm.savingError")}
        </p>
      )}
      ...
      <DialogFooter className="pt-4">
        <Button
          ...
          disabled={isPending}
        >
          {t("reportsOnboarding.googleSearchConsoleConfigForm.cancelButton")}
        </Button>
        <Button type="submit" disabled={isPending}>
          {isPending
            ? t("reportsOnboarding.googleSearchConsoleConfigForm.savingButton")
            : t("reportsOnboarding.googleSearchConsoleConfigForm.saveButton")}
        </Button>
      </DialogFooter>
    </form>
  );
