...
export function GitHubConfigForm({
  ...
  // The repo is stored on the Reports connection's state
  ...
  const cdConnectionId = WellKnownOrgMCPId.REPORTS(org.id);
  ...
  const selectedQuery = useQuery({
    queryKey: KEYS.reportsCompanionGithubSelected(org.id, connectionId),
    queryFn: async () => {
  ...
  const reposQuery = useQuery({
    queryKey: KEYS.reportsCompanionGithubRepos(
      org.id,
      connectionId,
  ...
  const saveMutation = useMutation({
    mutationFn: async (githubRepo: string) => {
      ...
      const agentId = getReportsAgentId(org.id);
      ...
      if (!repoConnectionId) {
        ...
        if (!installation) {
          throw new Error(
            t("reportsOnboarding.githubConfigForm.noGithubInstallation", {
              owner,
            }),
          );
        }
        ...
        if (!sourceConnection) {
          throw new Error(
            t("reportsOnboarding.githubConfigForm.githubConnectionNotFound"),
          );
        }
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: KEYS.reportsConnection(org.id, cdConnectionId),
      });
      await queryClient.invalidateQueries({
        queryKey: KEYS.reportsCompanionConnectionsPrefix(org.id),
      });
  ...
  const searchWarning =
    reposQuery.isError || allAccountsFailed
      ? t("reportsOnboarding.githubConfigForm.searchFailedTotal")
      : failedAccounts > 0
        ? t("reportsOnboarding.githubConfigForm.searchFailedPartial")
        : null;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {reposQuery.isLoading ? (
        <div className="flex min-h-[160px] items-center justify-center">
          <LoadingIndicator
            label={t("reportsOnboarding.githubConfigForm.loadingRepositories")}
          />
        </div>
      ) : (
        <>
          ...
          {options.length === 0 ? (
            !searchWarning && (
              <p className="text-sm text-muted-foreground">
                {t("reportsOnboarding.githubConfigForm.noRepositoriesFound")}
              </p>
            )
          ) : (
            <Form {...form}>
      ...
      {saveMutation.error && (
        <p role="alert" className="text-sm text-destructive">
          {saveMutation.error instanceof Error
            ...
            : t("reportsOnboarding.githubConfigForm.failedToSave")}
        </p>
      )}
      ...
      <DialogFooter className="pt-4">
        <Button
          ...
          disabled={isPending}
        >
          {t("reportsOnboarding.githubConfigForm.cancel")}
        </Button>
        <Button type="submit" disabled={isPending}>
          {isPending
            ? t("reportsOnboarding.githubConfigForm.saving")
            : t("reportsOnboarding.githubConfigForm.save")}
        </Button>
      </DialogFooter>
    </form>
  );
