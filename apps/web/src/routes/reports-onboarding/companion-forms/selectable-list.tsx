      ...
      {hideSearch ? null : (
        <div className="relative">
          ...
          <Input
            ...
            placeholder={
              searchPlaceholder ??
              t("reportsOnboarding.selectableList.searchPlaceholder")
            }
            aria-label={t("reportsOnboarding.selectableList.searchAriaLabel", {
              label: ariaLabel,
            })}
            className="h-8 pl-8"
          />
      ...
      >
        {filtered.length === 0 ? (
          <div className="px-2 py-2 text-sm text-muted-foreground">
            {t("reportsOnboarding.selectableList.noResults")}
          </div>
        ) : (
          filtered.map((option) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex w-full items-center justify-between gap-2 classic:rounded-sm compact:rounded-lg px-2 py-2 text-left text-sm",
