...
export function useReportsCompanions({
  selfClient,
  org,
  cdConnectionId,
  siteUrl,
  ...
  const schemaQuery = useQuery({
    queryKey: KEYS.reportsCompanionSchema(org.id, cdConnectionId),
    queryFn: async () => {
  ...
  const connectionsQuery = useSuspenseQuery({
    queryKey: KEYS.reportsCompanionConnections(org.id, connectionsKey),
    queryFn: async () => {
  ...
  const oauthStatusQueries = useSuspenseQueries({
    queries: oauthStatusConnections.map((connection) => ({
      queryKey: KEYS.reportsCompanionOAuthStatus(org.id, connection.id),
      queryFn: async () => {
  ...
  const registryQuery = useSuspenseQuery({
    queryKey: KEYS.reportsCompanionRegistry(org.id, registryKey),
    queryFn: async () => {
      // Suspense queries can't be `enabled`-gated; skip the call when there are
      // no binding requirements (schema resolved above, so this is stable).
      if (requirements.length === 0) {
  ...
  // Shared-SA connection status from reports — the source of truth
...
/** The reports connection's stored site URL (metadata.siteUrl).
...
export function useReportsSiteUrl({
  selfClient,
  org,
  cdConnectionId,
}: {
