    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Button
          ...
          aria-label={t("reportsOnboarding.companionCard.disconnectAriaLabel", {
            title,
          })}
        >
          {disconnecting ? (
      ...
      <TooltipContent>
        {t("reportsOnboarding.companionCard.disconnect")}
      </TooltipContent>
    </Tooltip>
  );
}
 ...
 *  needed — the binding lives in reports, not a Studio connection. */
          ...
          controls={
            <Tooltip>
              <TooltipTrigger asChild>
                <ConnectedConfigButton
                  ariaLabel={t(
                    "reportsOnboarding.companionCard.configureAriaLabel",
                    { title: card.title },
                  )}
              ...
              <TooltipContent>
                {t("reportsOnboarding.companionCard.editConfiguration")}
              </TooltipContent>
            </Tooltip>
          }
        />
  ...
  const trigger =
    variant === "configure" ? (
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <ConnectedConfigButton
            ariaLabel={t("reportsOnboarding.companionCard.configureAriaLabel", {
              title: card.title,
            })}
        ...
        <TooltipContent>
          {savedConfigEntries.length > 0
            ? t("reportsOnboarding.companionCard.editConfiguration")
            : t("reportsOnboarding.companionCard.configure")}
        </TooltipContent>
      </Tooltip>
    );
...
export function CompanionConfigDialog({
  ...
  const disconnectMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast.success(
        t("reportsOnboarding.companionCard.disconnectedSuccess", {
          title: card.title,
        }),
      );
      onOpenChange(false);
    ...
    onError: () => {
      toast.error(t("reportsOnboarding.companionCard.disconnectError"));
    },
  });
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader className="flex-row items-center gap-3 space-y-0 text-left">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            ...
            <DialogDescription>
              {t("reportsOnboarding.companionCard.configureDescription", {
                title: card.title,
              })}
            </DialogDescription>
          </div>
