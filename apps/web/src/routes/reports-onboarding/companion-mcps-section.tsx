...
import { useReportsCompanions } from "./use-companions.ts";
...
function SectionIntro() {
  ...
  return (
    <h1 className="text-lg font-medium leading-6 text-foreground lg:text-xl lg:leading-7">
      {t("routes.reportsOnboarding.companionSection.title")}
    </h1>
  );
}
...
// Suspense fallback for the whole section. `useReportsCompanions` (and the MCP
    ...
    <div className={SECTION_CONTAINER_CLASS}>
      ...
      >
        <p className="text-sm text-foreground">
          {t("routes.reportsOnboarding.companionSection.loadError")}
        </p>
        <p className="mt-1 text-sm text-muted-foreground">
          {t("routes.reportsOnboarding.companionSection.loadErrorDescription")}
        </p>
        <Button
          type="button"
          variant="outline"
          ...
          onClick={onRetry}
        >
          {t("routes.reportsOnboarding.companionSection.retry")}
        </Button>
      </div>
    </div>
  );
...
function CompanionMcpsSectionContent({
  ...
  const { cards, saStatusUnavailable } = useReportsCompanions({
    selfClient,
    org,
    cdConnectionId,
    siteUrl,
    ...
    <div className={SECTION_CONTAINER_CLASS}>
      ...
      >
        ...
        {saStatusUnavailable && (
          ...
          >
            {t("routes.reportsOnboarding.companionSection.statusUnavailable")}
          </p>
        )}
