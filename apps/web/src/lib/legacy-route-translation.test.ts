...
describe("canonicalProjectPathFromLegacyAgents", () => {
  it("renames only the namespace while preserving encoded path segments", () => {
    expect(
      canonicalProjectPathFromLegacyAgents(
        "/acme/agents/vir_1/apps/connection/GET%2Forders",
      ),
    ).toBe("/acme/projects/vir_1/apps/connection/GET%2Forders");
    expect(
      canonicalProjectPathFromLegacyAgents(
        "/acme/agents/vir_1/views/name%3Fpart%23anchor",
      ),
    ).toBe("/acme/projects/vir_1/views/name%3Fpart%23anchor");
  });

  it("does not rewrite lookalike or already-canonical paths", () => {
    expect(
      canonicalProjectPathFromLegacyAgents("/acme/agent/vir_1"),
    ).toBeNull();
    expect(
      canonicalProjectPathFromLegacyAgents("/acme/projects/vir_1"),
    ).toBeNull();
    expect(
      canonicalProjectPathFromLegacyAgents("/acme/agents-old/vir_1"),
    ).toBeNull();
  });
});

describe("projectWorkspacePathHasChild", () => {
  it("does not mistake a reserved-looking organization slug for a child", () => {
    expect(projectWorkspacePathHasChild("/agents/agents/code")).toBe(false);
    expect(projectWorkspacePathHasChild("/agents/agents/code/")).toBe(false);
    expect(projectWorkspacePathHasChild("/projects/projects/code")).toBe(false);
  });

  it("recognizes canonical and legacy children below the project boundary", () => {
    expect(projectWorkspacePathHasChild("/acme/projects/vir_1/settings")).toBe(
      true,
    );
    expect(
      projectWorkspacePathHasChild("/agents/agents/code/site-editor/content"),
    ).toBe(true);
  });
});
...
describe("translateLegacyThreadRoute", () => {
  it("lands an unscoped legacy thread on Home", () => {
    expect(translateThread()).toEqual({
      ...
      search: { sidepanel: true, thread: THREAD },
    });
  ...
  it("keeps an explicit request to hide Chat on a bare thread link", () => {
    expect(translateThread({ sidepanel: false }).search).toEqual({
      sidepanel: false,
      thread: THREAD,
    });
  });

  it("preserves project identity for project destinations", () => {
    const rows = [
      ["board", "/$org/projects/$agentId/tasks/{-$taskKey}"],
      ["files", "/$org/library"],
  ...
  it("maps fixed agent views to their canonical route owners", () => {
    const rows = [
      ["preview", "/$org/projects/$agentId/site-editor"],
      ["site-editor", "/$org/projects/$agentId/site-editor"],
      ["content", "/$org/projects/$agentId/site-editor/content"],
      ["code", "/$org/projects/$agentId/site-editor/code"],
      ["settings", "/$org/projects/$agentId/settings"],
      ["instructions", "/$org/projects/$agentId/settings"],
      ["connections", "/$org/projects/$agentId/settings"],
      ...
      ["cdn", "/$org/projects/$agentId/cdn"],
      ["connect-sources", "/$org/projects/$agentId/connect-sources"],
    ] as const;
  ...
  it("maps parameterized and custom main values without a legacy hop", () => {
    const rows = [
      [
        ...
        "/$org/projects/$agentId/site-editor/code",
        { file: "src/app.tsx" },
      ],
      [
        "automation:auto_1",
        "/$org/projects/$agentId/automations/$automationId",
        {},
      ],
      [
        formatPinnedViewTabId("conn_1", "get_orders"),
        "/$org/projects/$agentId/apps/$connectionId/$toolName",
        {},
      ],
      [
        formatFileTabId("org-fs:outputs/thr/a.pdf"),
        "/$org/projects/$agentId/outputs/file",
        { key: "org-fs:outputs/thr/a.pdf" },
      ],
      [
        formatDeckTabId("decks/q3.html"),
        "/$org/projects/$agentId/outputs/deck",
        { path: "decks/q3.html" },
      ],
      [
        formatLibraryFileTabId("home/docs/a.md"),
        "/$org/projects/$agentId/library/file",
        { path: "home/docs/a.md" },
      ],
      ["custom-view", "/$org/projects/$agentId/views/$viewId", {}],
    ] as const;
...
describe("translateLegacyMainParam", () => {
  ...
  it("moves Site Editor children to nested routes", () => {
    expect(translateMain("preview")).toMatchObject({
      kind: "canonical",
      route: { to: "/$org/projects/$agentId/site-editor" },
    });
    expect(translateMain("content")).toMatchObject({
      kind: "canonical",
      route: { to: "/$org/projects/$agentId/site-editor/content" },
    });
    expect(translateMain("code")).toMatchObject({
      kind: "canonical",
      route: { to: "/$org/projects/$agentId/site-editor/code" },
    });
  });
...
describe("translateLegacyPanelRoute", () => {
  ...
  it("lets an explicit legacy main value override the old panel segment", () => {
    expect(
      ...
      route: { to: "/$org/projects/$agentId/settings" },
    });
  });
  ...
  it("lands a truncated parameterized panel on the owning agent", () => {
    ...
    ).toMatchObject({ route: { to: "/$org/projects/$agentId" } });
  });
...
describe("isCanonicalAgentIdSegment", () => {
  it("recognizes persisted and well-known agent id namespaces", () => {
    for (const id of [
      ...
      "studio-task-manager_org_1",
      "studio-automation-manager_org_1",
...
describe("translateLegacyOrgDestinationAgentSearch", () => {
  ...
  it("promotes project-capable destinations and strips Library identity", () => {
    for (const routePath of ["/$org/tasks/{-$taskKey}", "/$org/reports"]) {
      const result = translateLegacyOrgDestinationAgentSearch({
        org: ORG,
        routePath,
        search: { virtualmcpid: AGENT, sidepanel: false },
      });
      expect(result?.kind).toBe("canonical");
      if (result?.kind !== "canonical") continue;
      expect(result.route.to).toBe(
        routePath === "/$org/reports"
          ? "/$org/projects/$agentId/reports"
          : "/$org/projects/$agentId/tasks/{-$taskKey}",
      );
      expect(result.search).toEqual({ sidepanel: false });
    }

    expect(
      translateLegacyOrgDestinationAgentSearch({
        org: ORG,
        routePath: "/$org/library",
        search: { virtualmcpid: AGENT, sidepanel: false },
      }),
    ).toEqual({ kind: "same-route", search: { sidepanel: false } });
  });
  ...
  it("ignores canonical agent routes and searches with no legacy identity", () => {
    expect(
      translateLegacyOrgDestinationAgentSearch({
        org: ORG,
        routePath: "/$org/projects/$agentId/settings",
