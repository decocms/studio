...
function BrowseButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs text-muted-foreground transition-colors hover:border-foreground/30 hover:text-foreground cursor-pointer"
    >
      <SearchSm size={13} />
      {label}
    </button>
    ...
    >
      <div className="relative aspect-square">
        ...
        <button
          ...
          {...attributes}
          {...listeners}
          className="absolute left-1 top-1 flex h-6 w-6 cursor-grab items-center justify-center rounded-lg bg-background/80 text-muted-foreground opacity-0 backdrop-blur-sm transition-opacity hover:text-foreground active:cursor-grabbing group-hover/card:opacity-100"
        >
          <DotsGrid size={13} />
        </button>
        <button
          ...
          onClick={onRemove}
          className="absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-lg bg-background/80 text-muted-foreground opacity-0 backdrop-blur-sm transition-opacity hover:text-destructive group-hover/card:opacity-100 cursor-pointer"
        >
          <XClose size={13} />
        </button>
      </div>
        ...
        {sandboxRef ? (
          productId ? (
            ...
            <button
              type="button"
              onClick={() => setPickerOpen(true)}
              className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed py-3 text-sm text-muted-foreground transition-colors hover:border-foreground/30 hover:text-foreground cursor-pointer"
            >
              <SearchSm size={14} />
