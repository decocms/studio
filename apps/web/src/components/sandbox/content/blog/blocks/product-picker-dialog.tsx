...
function ProductRow({
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full items-center gap-3 classic:rounded-md compact:rounded-lg border px-2 py-2 text-left transition-colors",
        selected
  ...
  if (selected) {
    return (
      <button
        ...
        className="flex items-center gap-1.5 classic:rounded-md compact:rounded-lg px-2 py-1.5 text-sm text-muted-foreground hover:bg-muted/60 hover:text-foreground"
      >
        <ChevronLeft size={14} />
          ...
          <div className="p-1">
            {options.map((option) => (
              <button
                ...
                className="block w-full truncate classic:rounded px-2 py-1.5 text-left text-sm hover:bg-muted/60 compact:rounded-lg"
              >
                {option.label}
              </button>
            ))}
        ...
        <div className="flex shrink-0 items-center gap-1 border-b border-border px-4 py-2">
          {MODES.map((m) => (
            <button
              ...
              className={cn(
                "classic:rounded-md compact:rounded-lg px-2.5 py-1 text-sm transition-colors",
                mode === m.id
