...
export function PreviewContent({ virtualMcpId }: { virtualMcpId: string }) {
  ...
  const urlGroup = showPreviewToolbar ? (
    <>
      ...
      {/* Open-in-new-tab only makes sense for a real sandbox iframe — the
          production / Fast Preview surface is a cross-origin published site or
          an ephemeral `/live/previews` draft URL, neither of which is a stable
          link to hand out. */}
      {display.mode === "sandbox" && (
        <Tooltip>
          <TooltipTrigger asChild>
