/**
 * The inbox: a popover beside the settings gear listing unread updates on the
 * tasks you follow, then the release notes. Selecting a task update opens its
 * card; selecting a release opens it inline behind a back button.
 *
            ...
            if (item.type === "task") {
              ...
              return (
                <InboxTaskItem
                  ...
                  actor={
                    item.update.actorId
                      ? memberByUserId.get(item.update.actorId)
                      : undefined
                  }
                  onSelect={() => {
                    onClose();
...
/** The dot count without mounting the panel — the trigger renders on every
 *  page, the list only when opened. */
function useInboxDotCount(): number {
  return useInboxFeed().redDotCount;
}
