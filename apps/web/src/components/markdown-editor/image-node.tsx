    ...
    <NodeViewWrapper className="group/image relative my-3 w-fit max-w-full">
      ...
      {editor.isEditable && (
        <button
          ...
          onClick={deleteNode}
          className="absolute right-2 top-2 flex size-7 items-center justify-center classic:rounded-md compact:rounded-lg border border-border bg-card text-muted-foreground opacity-0 shadow-sm transition-opacity hover:text-foreground focus-visible:opacity-100 group-hover/image:opacity-100"
        >
          <X size={14} />
        </button>
      )}
