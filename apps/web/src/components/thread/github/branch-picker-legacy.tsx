...
export function BranchPickerLegacy({
  orgId,
  orgSlug,
  userId,
  userLabel,
  ...
  // Seed the active item with the current branch so cmdk opens focused on it.
  ...
  // userId -> user for avatars; non-suspense so the trigger never blocks.
  ...
  // Open PRs — a PR is just a branch; picking one starts on its head branch.
  const {
    data: prs = [],
  ...
  // Only same-repo PRs are openable (a fork's head.ref isn't in this repo).
    ...
    <Popover
      ...
      onOpenChange={
        disabled
          ? undefined
          : (next) => {
              // Re-seed the highlight + clear stale search each time it opens.
              if (next) {
