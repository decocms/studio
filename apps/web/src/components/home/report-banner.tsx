/**
 * Home report banner — surfaces the Reports diagnostic on the
...
} from "@/hooks/use-reports-diagnostic";
import {
  type ReportBannerStatus,
  deriveReportBannerStatus,
} from "@/hooks/reports-diagnostic-status";
...
function BannerShell({
  ...
  status: Exclude<ReportBannerStatus, "none">;
  host: string | null;
  onOpen: () => void;
}) {
  const t = useT();
  const generating = status === "generating";
  const store = host ?? t("reports.banner.storeDefault");
      ...
      <div className="relative flex min-h-28 items-center gap-4 px-5 py-5 sm:h-36 sm:gap-6 sm:pr-6 sm:pl-52">
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <span className="text-base font-medium leading-6 text-foreground sm:text-lg">
            {generating
              ? t("reports.banner.generatingTitle")
              : t("reports.banner.readyTitle")}
          </span>
          <span className="truncate text-sm text-muted-foreground">
            {generating
              ? t("reports.banner.generatingSubtitle", { store })
              : t("reports.banner.readySubtitle", { store })}
          </span>
        </div>
        <div
          className={cn(
...
function ReportBannerInner() {
  const { org } = useProjectContext();
  const navigate = useNavigate();
  const { diagnostic, isSuccess, host, connectionId } = useReportsDiagnostic();

  const status = isSuccess ? deriveReportBannerStatus(diagnostic) : "none";
  ...
  const openReport = () => {
    ...
    navigate(reportNavTarget(org, connectionId));
  };
