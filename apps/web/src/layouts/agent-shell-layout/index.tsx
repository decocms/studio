 ...
 *             └── WorkspacePanelGroup
 *                 ├── Chat panel  (header: threads + new chat, plus Main's
 *                     recovery control when route or history state hides Main)
 *                 └── Main panel  (header: Chat visibility, view tabs,
 *                     Preview controls, publish)
 *
...
import { resolveMobileSurface } from "@/hooks/workspace-panel-state";
...
import { MainPanelTabsProvider } from "@/layouts/main-panel-tabs/main-panel-tabs-provider";
...
export default function AgentShellLayout() {
   ...
   * a valid one) while a compatibility redirect is still resolving. Wait only
   * when workspace identity changes: ordinary page/file navigation must keep
   * editors mounted, including their DOM-owned Monaco instances. */
  const routeCommitPending = useRouterState({
    select: (state) => {
      const previous = state.resolvedLocation;
      const next = state.location;
      if (!state.isLoading || !previous || previous.href === next.href) {
        return false;
      }
      const currentParams = router.getMatchedRoutes(
        previous.pathname,
      ).routeParams;
      const nextParams = router.getMatchedRoutes(next.pathname).routeParams;
      return (
        ["org", "agentId", "taskId", "_splat"].some(
          (key) => currentParams[key] !== nextParams[key],
        ) ||
        (["thread", "virtualmcpid", "main"] as const).some(
          (key) => previous.search[key] !== next.search[key],
        )
      );
    },
  });

  if (routeCommitPending && router.latestLoadPromise) {
    throw router.latestLoadPromise;
  }
