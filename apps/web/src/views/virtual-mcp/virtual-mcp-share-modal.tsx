...
function studioClientHeaders(client: string): Record<string, string> {
  return {
    "x-studio-client": client,
    // Compatibility with Studio releases from before the header rename.
...
function InstallCursorButton({ url, serverName, agentId }: ShareWithNameProps) {
  ...
  const handleInstall = () => {
    ...
    const connectionConfig = {
      type: "http",
      url: url,
      headers: studioClientHeaders("Cursor"),
    };
...
function InstallClaudeButton({ url, serverName, agentId }: ShareWithNameProps) {
  ...
  const handleInstall = async () => {
    ...
    const connectionConfig = {
      type: "http",
      url: url,
      headers: studioClientHeaders("Claude Code"),
    };
...
function EnvVarsBlock({
  ...
  const [copied, setCopied] = useState(false);
  const studioUrl = window.location.origin;
  const keyLine = apiKey
    ? `STUDIO_API_KEY=${apiKey}`
    : `STUDIO_API_KEY=<api-key>`;
  const urlLine = `STUDIO_BASE_URL=${studioUrl}`;
