...
function VirtualMcpDetailViewWithData({
  ...
  const persistForm = async () => {
    ...
    // They stay in form state so the user keeps editing, but the request body
    // only carries entries the server schema accepts.
  ...
  const settingsGroups: ProjectSettingsGroupDef[] = [
    {
      ...
      rows: [
        ...
        },
        {
          key: "site",
          icon: <Cube01 size={16} />,
          title: t("virtualMcp.settings.site.classicTitle"),
  ...
  const connectButton = (
    <Button
      ...
      size="sm"
      onClick={() => {
        track("agent_connect_modal_opened", { agent_id: virtualMcp.id });
  ...
  return (
    <Page>
      {compact ? (
        <>
          <Page.Title>{t("virtualMcp.settings.title")}</Page.Title>
          <ProjectSettingsTabs section={section ?? "general"} />
          <Page.Actions
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 px-4 text-xs text-muted-foreground">
                      ...
                      <span>
                        ...
                        {format(new Date(virtualMcp.created_at), "PP", {
                          locale,
                        })}
                      </span>
                      <span className="text-muted-foreground/50">·</span>
              ...
              <div className="flex flex-col gap-10">
                ...
                {section === null ? (
                  ...
                  <ProjectSettingsDetail
                    title={t(
                      section === "site"
                        ? "virtualMcp.settings.site.classicTitle"
                        : PROJECT_SETTINGS_SECTIONS[section].titleKey,
                    )}
                              ...
                              <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    className="absolute top-2 right-2 h-7 w-7 text-muted-foreground"
                                    onClick={() =>
                                      setInstructionsFullscreen(true)
                                    }
                                    aria-label={t(
                                      "virtualMcp.virtualMcp.openFullscreenEditor",
                                    )}
                ...
                (Fast Preview renders against that URL, so it needs one). */}
                    ...
                    {section === "site" && (
                      <SettingsSection
                        title={t("virtualMcp.virtualMcp.sandbox")}
                      >
                        <SettingsCard>
                          <SettingsCardRow>
                            <RepoRow repo={runtimeCardRepo} />
