...
import { getPrompt, useMCPClient, useProjectContext } from "@/sdk";
...
} from "@/components/chat/dialog-prompt-arguments";
import { useThreadActions } from "@/components/chat/store/hooks";
import { createMentionDoc } from "@/components/chat/tiptap/mention/node";
...
export interface UseStartThreadFromPromptResult {
   ...
   */
  startBlank: (opts?: { panel?: string }) => Promise<void>;
...
export function useStartThreadFromPrompt({
  ...
  const startBlank = async (opts?: { panel?: string }) => {
    if (inFlightRef.current) return;
    // oxlint-disable-next-line ban-ref-current-assignment/ban-ref-current-assignment -- in-flight guard for duplicate-click prevention
    inFlightRef.current = true;
    setStarting(true);
    try {
      ...
      setTaskId(
        newId,
        agentId,
        opts?.panel ? { panel: opts.panel } : undefined,
      );
    } catch (error) {
