...
- **Member**: Basic access to view and use existing connections (great for team members who just need to work with what's already set up)
...
2. Click **Create Role**
3. Give it a name (like "Developer" or "Support Team")
4. Select which actions this role should be allowed to perform

Then assign the role to people from the **Members** page.
...
For organizations on an invoiced (contract) plan, access is billed per seat. The **Members** page shows a **Seat** column where an Owner or Admin can toggle team members on or off a paid seat.

Toggling seats doesn't apply immediately — changes stage locally first. An apply bar shows the pending change count and the resulting paid-seat total; review it, then commit the batch to update billing for everyone at once.

<Callout type="tip">
  Self-serve, pay-as-you-go seat billing (with an in-product checkout) is rolling out — until it's available for your organization, the Seat column only appears for orgs on an invoiced plan.
</Callout>
...
- **Members page**: Invite teammates, assign roles, and (for invoiced orgs) manage seats
- **Roles page** (**Settings → Roles**): Create and edit custom roles
...
3. Click their current role (probably "Member")
4. Select **Admin**
...
5. From the **Members** page, assign this role to the appropriate team members
