...
<Callout type="note">
  **A first install against an empty database migrates before it serves.** Studio takes a Postgres advisory lock around the DBOS system-schema migration, so whichever process starts first migrates and the others wait — no replica-count juggling is needed, with or without Argo CD. The chart also ships a single-writer migration Job (`migrateJob.enabled`, on by default) that gives that cold migration somewhere to finish: it has no liveness probe, whereas an API container migrating during boot is killed at roughly 60 seconds while a cold run takes minutes. Under plain `helm install` the Job starts alongside the first pods, so expect those pods to restart a few times during the first install. It converges on its own and nothing is corrupted. Under Argo CD the sync wave holds the Deployments until the Job finishes, so you will not see the restarts at all.
</Callout>
