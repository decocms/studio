...
A production deployment uses four Helm releases:
...
| `sandbox-env` | Once per Studio environment | Installs the environment's `SandboxTemplate`, sentinel Secret, and optional warm pool, preview Gateway, and idle housekeeper |
| `sandbox-controller` | Once per Studio environment | Serves the mTLS claim API Studio calls to start and stop sandboxes, holds the cluster RBAC for claims, owns `sandbox_runner_state` in Studio's database, and reconciles `SandboxVariant` image variants |

The operator is shared cluster infrastructure. Each `sandbox-env` release is isolated by `envName`, so production, staging, and other Studio releases can share the operator and the `agent-sandbox-system` namespace without resource-name collisions. Each environment also gets its own controller release, named after the release; exactly one release per namespace reconciles `SandboxVariant`s (`variants.enabled`), and the others only serve claims.

When a hosted agent needs a sandbox, Studio calls the sandbox controller. The controller creates a `SandboxClaim`, the operator turns it into an ephemeral sandbox pod from the environment's `SandboxTemplate`, and the controller tells Studio where the pod's daemon is and which bearer opens it. Studio then talks to the daemon directly through its in-cluster Service; control traffic never passes through the controller. If preview routing is enabled, the controller also creates one `HTTPRoute` per claim so the wildcard Gateway routes directly to the sandbox Service on port 9000. Studio itself holds no Kubernetes RBAC for sandboxes.

<Callout type="warning">
  Keep the controller's claim API inside the cluster. mTLS is its only authentication, and a caller holding a certificate from its CA can start sandboxes with arbitrary environment variables. The chart's NetworkPolicy admits only Studio's pods.
</Callout>
...
- A way to issue the Studio/controller mTLS certificates from one dedicated CA; the examples below use [cert-manager](https://cert-manager.io)
...
These are the first versions that run sandboxes through the controller: `chart-deco-studio` 0.17.0 adds `sandboxController`, and `sandbox-env` 0.23.0 drops the Role it used to grant Studio.
...
### 2. Issue the Studio/controller certificates

Studio and the controller authenticate each other with mTLS in both directions: Studio presents its certificate to the claim API, and the controller presents its certificate when it calls Studio back to re-mint clone and org-fs credentials. Issue both leaves from one CA dedicated to this pair, each with the server and client auth usages, because each side serves with its leaf and calls the other with it. The DNS names are the Service names each side dials.

With cert-manager, a self-signed root, a CA `ClusterIssuer`, and one `Certificate` per side:
...
metadata:
  ...
  namespace: cert-manager # the CA issuer reads its Secret from here
spec:
  isCA: true
...
cert-manager writes `tls.crt`, `tls.key`, and `ca.crt` into each Secret, which is the shape both charts read. Create the `deco-studio` namespace before applying, since the Studio certificate lives there.

### 3. Configure and install Studio
...
Create `studio-values.yaml`. The Studio release name and namespace must match the certificate's DNS name and the controller's callback URL below.
...
# The controller connection. Required with STUDIO_AGENT_SANDBOX_ENABLED:
# the chart refuses to render one without the other.
sandboxController:
  enabled: true
  ...
  callbackPort: 9443 # exposed as the Service port `sandbox-callbacks`

secret:
  secretName: deco-studio-secrets
```

The chart turns `sandboxController` into Studio's `STUDIO_SANDBOX_CONTROLLER_URL`, `STUDIO_SANDBOX_CONTROLLER_TLS_CERT`, `STUDIO_SANDBOX_CONTROLLER_TLS_KEY`, `STUDIO_SANDBOX_CONTROLLER_CA`, and `STUDIO_SANDBOX_CONTROLLER_CALLBACK_PORT`. Outside the chart, set those variables yourself: with `STUDIO_AGENT_SANDBOX_ENABLED=true`, Studio refuses to boot unless the URL is `https://` and the certificate, key, and CA paths are all set. Studio needs no ServiceAccount or cluster RBAC for sandboxes.
...
### 4. Install the per-environment sandbox resources
...
This creates `SandboxTemplate/studio-sandbox-production` (plus its `-medium` variant) and the sentinel Secret `studio-sandbox-sentinel-production`.

### 5. Install the sandbox controller

The controller reads Studio's database, where it owns `sandbox_runner_state`. Give it the same `DATABASE_URL` in its own namespace:
...
Create `controller-values.yaml`:

```yaml
claims:
  enabled: true
  # The controller's leaf; its ca.crt also verifies Studio in both directions.
  tlsSecretName: sandbox-controller-tls
  clientCASecretName: sandbox-controller-tls
  studioCASecretName: sandbox-controller-tls
  # Studio's `sandbox-callbacks` Service port.
  studioCallbackUrl: "https://deco-studio.deco-studio.svc:9443"
  database:
    secretName: sandbox-controller-db
  # The sandbox-env release of this environment.
  sandboxTemplate: studio-sandbox-production
  envName: production
  # Required in a cluster: with a pattern, Studio reaches daemons by Service DNS.
  # It is also the preview URL Studio shows (see Preview Gateway below).
...
The chart installs the `SandboxVariant` CRD, the controller Deployment, a `ClusterIP` Service on port 8443, a NetworkPolicy that admits only Studio's API and worker pods (`claims.studio`), and a Role in `agent-sandbox-system` that lets it manage claims, patch the sandbox Services, and manage per-claim `HTTPRoute` objects. `claims.envName` is stamped as `studio.decocms.com/env` on every claim, pod, and route; the housekeeper scopes its sweeps by that label. Without `claims.previewUrlPattern` the controller port-forwards to daemons, which only works when Studio runs on the same host.
...
Warm pools are disabled by default because every idle warm pod consumes the full sandbox resource request. Enabling one also requires the controller and the sandbox template to share a sentinel token. The controller uses it only for the first daemon configuration request and immediately rotates the daemon to a per-claim token.

`sandbox-env` generates the token into `studio-sandbox-sentinel-<envName>` and preserves it across upgrades. The controller runs in the same namespace, so it can read that Secret directly:
...
Without the sentinel, the controller provisions every claim cold and cannot use the pool. Set `sentinel.token` on `sandbox-env` only when another release needs the same value; source it from your secret manager, not a values file. Tenant pools (`claims.tenantPools`) also need the sentinel.
...
Studio renews claims while someone is using them, while the optional housekeeper CronJob cleans up idle or unrecoverable claims and orphaned routes/pods. Its defaults run every five minutes and reap claims idle for 15 minutes. Point it at the controller's idle TTL so the two cannot drift:

```yaml
housekeeper:
  enabled: true
  schedule: "*/5 * * * *"
  # Rendered by the sandbox-controller release from claims.idleTtlSeconds.
  idleTtlConfigMap: sandbox-controller-idle-ttl
```

The controller's `claims.envName` is required when the housekeeper is enabled, because the housekeeper's default selectors are environment-scoped. Without the matching label, it intentionally selects no claims.
...
Without a preview Gateway, Studio can use its legacy in-process preview proxy when `STUDIO_SANDBOX_PREVIEW_URL_PATTERN` is set on Studio and the wildcard host routes to Studio. The current production design uses Gateway API and routes each preview directly to its sandbox daemon:
...
Then point the controller at it, so it attaches one `HTTPRoute` per claim:
...
## Main sandbox controller values

These defaults come from `deploy/helm/sandbox-controller/values.yaml`:

| Parameter | Description | Default |
| --- | --- | --- |
| `image.repository` / `image.tag` | Controller image | `ghcr.io/decocms/studio/sandbox-controller` / the chart's `appVersion` |
| `sandboxNamespace` | Namespace of the templates, variants, and claims | `agent-sandbox-system` |
| `variants.enabled` | Reconcile `SandboxVariant`s; one release per namespace | `true` |
| `claims.enabled` | Serve the claim API Studio calls | `false` |
| `claims.tlsSecretName` / `claims.clientCASecretName` | Server certificate / CA of Studio's client certificate | Required with claims |
| `claims.studioCallbackUrl` / `claims.studioCASecretName` | Studio's callback listener / its CA; empty URL disables credential re-mints | `""` |
| `claims.database.secretName` | Secret with Studio's `DATABASE_URL` | Required with claims |
| `claims.sandboxTemplate` / `claims.envName` | The `sandbox-env` template and environment label | `studio-sandbox` / `""` |
| `claims.previewUrlPattern` / `claims.gateway` | Preview URL and the Gateway per-claim routes attach to | `""` |
| `claims.sentinel.secretName` | Warm-pool sentinel Secret | `""` |
| `claims.idleTtlSeconds` / `claims.deleteDeadline` | Idle window / how long a delete waits before answering "still draining" | `900` / `30s` |
| `claims.studio.namespace` / `claims.studio.podSelector` | Studio pods the NetworkPolicy admits | Required / API and worker pods |

## Upgrades and removal

Render and lint the charts before applying changes:
...
<Callout type="warning">
  **Moving an existing install to the controller is a drain, not a rolling deploy.** A Studio that still runs its own sandbox runner and the controller would both manage the same claims. Upgrade Studio and install the controller, let the old Studio pods finish, and only then upgrade `sandbox-env` to 0.23.0, which removes the Role the old Studio used.
</Callout>
...
The chart carries an optional `preview` block. It turns a release into a
short-lived, self-contained environment: an ephemeral Postgres in the same
namespace, migrations run once by a Job instead of racing across pods, and an
`HTTPRoute` onto a shared wildcard `Gateway`. It is meant for reviewing a change
before it merges — one throwaway Studio per pull request.

`preview.enabled` is `false` by default and every preview template is gated on
it, so a normal install renders exactly what it rendered before. You do not need
this to self-host Studio, and you should leave it off unless you are building a
review pipeline.

Turning it on requires an orchestrator to create and destroy an environment per
pull request. The suggested one is Argo CD, and the `chart-deco-studio-previews`
chart in this repository ships that half: an `ApplicationSet` whose pullRequest
generator watches a repository for a label, plus the shared `Gateway`. Point it
at your own repository, domain and object storage — it has no defaults.
...
OAuth sign-in, billing, the monitoring dashboard, outbound email, or anything
that depends on more than one pod. It also authenticates nobody by itself — put
an authorization policy on the Gateway listener before exposing it, because a
preview accepts email/password signup and can drive LLM agents.
