...
<Callout type="warning">
  **First install against an empty database under plain Helm: install with no app pods, then turn the app on** — `--set replicaCount=0 --set worker.replicaCount=0 --set worker.autoscaling.enabled=false`, then `helm upgrade` with your real values. All three are needed: a pod runs two API containers (`api-0`, `api-1`), so `replicaCount: 1` is already two processes, and `worker.replicaCount: 0` is a no-op while the worker HPA's `minReplicas: 2` is active. Every process migrates the DBOS system schema on boot, and the DBOS SDK bumps its schema version with an `UPDATE` that has no `WHERE` clause, assuming `dbos.dbos_migrations` holds a single row. Two replicas reaching that point together leave two rows, after which every bump collides on the primary key and the version can never advance — the pods `CrashLoopBackOff` permanently and only a manual `DELETE` recovers the database. The chart ships a single-writer migration Job (`migrateJob.enabled`, on by default) that closes this under Argo CD, which holds the Deployments until the Job finishes; plain `helm install` starts the Job alongside the first pods, so the replica count is what protects you. Upgrades are safe at any replica count.
</Callout>
