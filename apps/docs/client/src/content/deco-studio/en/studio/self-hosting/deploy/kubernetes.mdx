...
- **Auth Config Secret** (`deco-mcp-mesh-auth-secrets`): Contains OAuth client IDs/secrets, API keys, and deployment-wide SSO env vars (`AUTH_SSO_DOMAIN`, `AUTH_SSO_MS_*`, `AUTH_SSO_GOOGLE_*`)

For deployment-wide enforced SSO (single IdP for the whole instance), pass the
SSO envs through the auth-config Secret. See
[Authentication → Deployment-wide SSO](/deco-studio/self-hosting/authentication)
