---
title: Tools
description: What Super Agent can do, and the two kinds of tools it has access to
icon: Wrench
---

## Two kinds of tools

Super Agent has access to two kinds of tools:
...
These are the tools the Super Agent agent uses internally to manage its own work — not a public HTTP API you call from outside. For the external, "bring-your-own-client" surface (MCP endpoints, the Super Agent thread REST API), see the [API Reference](/en/studio/api-reference/connection-proxy).
...
When an agent has a sandbox (a linked GitHub repo, see [Agents](/en/studio/agents)), Super Agent is also given six **VM tools** that operate inside that sandbox:
...
## How Super Agent uses tools

Super Agent starts each task with only its built-in tools. It sees scope tools listed in its instructions and uses them as needed to complete your request.
...
**Next:** See [Context and tasks](/en/studio/decopilot/context-and-tasks) to understand how Super Agent manages memory.
