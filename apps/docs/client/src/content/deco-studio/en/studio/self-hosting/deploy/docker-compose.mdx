...
- **`docker-compose.dev.yml`** — **development**: only the dependencies (PostgreSQL + a 3-node NATS cluster, prod-like topology); you run the API and web apps **from source** with `bun run dev:servers`. `dev:servers` starts no sandbox controller, so hosted sandboxes need one configured through `STUDIO_SANDBOX_CONTROLLER_*`. `bun run dev` starts one itself on the controller's Docker runtime when `STUDIO_AGENT_SANDBOX_ENABLED=true` and no controller URL is set; for sandboxes on Kubernetes, see the [dev-hybrid](https://github.com/decocms/studio/tree/main/selfhost/examples/dev-hybrid) loop.
