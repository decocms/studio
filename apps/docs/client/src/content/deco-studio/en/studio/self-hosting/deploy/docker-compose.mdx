...
Studio uses **PostgreSQL** as its database. The repo ships two compose files under `deploy/docker-compose/`:

- **`docker-compose.postgres.yml`** — Studio **plus a fully bundled stack** (PostgreSQL + NATS + MinIO), all wired so the app provisions nothing itself. The recommended starting point. (`selfhost/examples/docker-compose` wraps this with a documented `.env.example`.)
- **`docker-compose.yml`** — Studio only, expecting a PostgreSQL you provide (a managed instance, or one running on your machine).

Both use a Docker volume for persistence and an automatic `/health` check.
...
For richer configuration (SAML/SSO, email providers, magic links), mount an `auth-config.json` into the container at `/app/apps/mesh/auth-config.json` by adding a volume to the `studio` service, using [`apps/mesh/auth-config.example.json`](https://github.com/decocms/studio/blob/main/apps/mesh/auth-config.example.json) as a starting point. See [Authentication](/en/studio/self-hosting/authentication) for the full reference.
