...
- **`docker-compose.dev.yml`** — **development**: only the dependencies (PostgreSQL + a 3-node NATS cluster, prod-like topology); you run Studio **from source** with `bun run --cwd=apps/mesh dev:server`. No sandbox here — that needs Kubernetes (see the [dev-hybrid](https://github.com/decocms/studio/tree/main/selfhost/examples/dev-hybrid) loop).
