...
<Callout type="warning">
  The CloudEvents pub/sub feature was removed, including its worker, delivery queue, and `EVENT_PUBLISH`/`EVENT_SUBSCRIBE` tools. The `ON_EVENTS` subscriber contract remains. NATS still supports streaming, broadcasts, and shared state; the SSE hub remains under `apps/api/src/event-bus/`.
</Callout>
...
- **NATS supports streaming and cross-pod coordination.** The CloudEvents pub/sub feature was removed.
- **Sandbox routing is surface-owned**: hosted Studio uses AgentSandbox; the native app intercepts locally and records `local-api`.
