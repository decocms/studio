...
The native desktop app wraps the same web surface and adds account-scoped local
terminal sessions. Those terminals are separate from hosted chat execution;
they are not a chat runtime or sandbox provider.
...
| **NATS** | Live messaging infrastructure for JetStream `/stream` fan-out (`decopilot.stream.<thread>`) → UI live tail and cross-pod run signals. |

<Callout type="warning">
  **The event bus is dormant.** The CloudEvents pub/sub *feature* (`EVENT_PUBLISH`/`EVENT_SUBSCRIBE`, the durable event queue, `ON_EVENTS` subscribers) is only consumed by the workflow plugin, which is not in use. NATS itself is **not** dormant — it serves the live traffic listed above. Don't conflate the two.
</Callout>
...
The run executes in-process on the worker with **Decopilot** and uses a managed
**agent sandbox** when code execution is required.
...
| **Daemon API** (`/_sandbox/*`) | Bearer `DAEMON_TOKEN` | Control surface: fs ops (read/write/edit/bash/grep), git (status/diff/publish), exec scripts, setup (clone → install → start), tasks, and SSE events. | The cluster (worker for agent fs/git/bash tools; API for UI setup + events) |
...
The managed sandbox uses a privileged sidecar to mount the filesystem and
propagate it into the main container; the unprivileged daemon cannot mount it
directly.
...
- **Agent sandboxes** expose a public preview and a token-protected control API in Kubernetes.
