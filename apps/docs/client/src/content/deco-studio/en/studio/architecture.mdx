...
- **Desktop** — the native Studio app on the user's laptop, where its embedded `local-api` owns native coding harnesses and local sandboxes.
...
| Component | Role |
| --- | --- |
| **DB** (PostgreSQL, via Kysely) | System of record: orgs, connections, credential vault, audit, threads + messages, and `sandbox_runner_state` (written only by the sandbox controller). It also holds the **DBOS queues and `workflow_status` journal** that make runs durable and recoverable. |
...
3. A **Worker** dequeues the gate workflow and starts a hosted child workflow. In-process Decopilot runs use `HOSTED_HARNESS_QUEUE`; sandbox-hosted Claude Code runs use `HOSTED_HARNESS_SANDBOXED_QUEUE`. The parent gate doesn't wait on the child — it live-tails the same NATS stream the child publishes to, while the durable projector writes messages and terminal status.
...
Both paths are hosted and go through the **sandbox controller** when they need a sandbox:
...
Studio does not create hosted sandboxes itself. The API and the workers call the **sandbox controller**, a separate Go service in the sandbox namespace, over an mTLS claim API. The controller holds the Kubernetes RBAC, creates the `SandboxClaim`s, owns `sandbox_runner_state`, and answers where each sandbox's daemon is and which bearer opens it. Daemon and preview traffic then goes straight from Studio or the browser to the pod; it never passes through the controller. The controller calls back to Studio over mTLS to re-mint clone and org-fs credentials. It must never be reachable from the internet: a caller holding a certificate from its CA can start sandboxes with arbitrary environment variables. On a single machine without Kubernetes, the same controller serves sandboxes as Docker containers (its `docker` runtime).
...
| --- | --- | --- |
| Where | the sandbox controller + agent-sandbox operator: a `SandboxClaim` pod per (user, projectRef) | The native app's `local-api` and `SandboxManager`, with one local worktree per handle |
| Reached over | the daemon address the controller reports: the in-cluster Service (control); ingress (preview) | loopback interception; `<handle>.localhost:<port>` (preview) |
| Routing owner | Studio's hosted API always goes through the sandbox controller and reports `agent-sandbox` | The native app intercepts the same lifecycle/filesystem calls locally and reports `local-api` |
...
- **NATS supports streaming and cross-pod coordination.**
- **Sandbox routing is surface-owned**: hosted Studio goes through the sandbox controller; the native app intercepts locally and records `local-api`.
- **Studio holds no cluster access for sandboxes**: the controller creates claims and owns their state.
