...
- **Reports** — organization-wide reports and diagnostics.
- **Tasks** — the organization's task board.
...
- **Projects** — opens a project's own Home, Reports, Tasks, Site Editor, and other enabled views.

### The agent workspace

Open an agent — or start a thread with one — and the screen splits in two:

- **Left — main view.** The project sidebar selects durable destinations such as Home, Reports, Tasks, Site Editor, Settings, and Automations. Site Editor owns its contextual Preview, Content, and Code tabs.
- **Right — chat.** Talk to the agent in natural language; it uses its attached tools to do the work. Past messages live here. Above the input sit the **model-tier picker** and the **runtime picker** (where the agent runs — see [Agents](/en/studio/agents#where-an-agent-runs)).
