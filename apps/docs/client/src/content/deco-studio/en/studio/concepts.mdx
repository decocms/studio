...
- **Left — chat.** Talk to the agent in natural language; it uses its attached tools to do the work. Past messages live here. Above the input sit the **model-tier picker** and the **runtime picker** (where the agent runs — see [Agents](/en/studio/agents#where-an-agent-runs)).
- **Right — main view.** The project sidebar selects durable destinations such as Home, Reports, Tasks, Site Editor, Settings, and Automations. Site Editor owns its contextual Preview, Content, and Code tabs.
