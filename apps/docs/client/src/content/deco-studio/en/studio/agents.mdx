...
<Callout type="tip">
  A cloud sandbox is shared per branch across everyone in your organization who's authorized to work on that agent — if a teammate opens the same agent and branch, you're both working against the same running environment and file changes. Each teammate's local runtime (via `deco link`) stays scoped to their own machine.
</Callout>
