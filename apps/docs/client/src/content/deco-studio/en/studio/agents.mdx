...
- **Left — chat.** Talk to the agent in natural language; it uses its attached tools to do the work. Your past messages and threads with this agent live here. Above the input is the **model-tier picker** (Fast, Smart, Thinking — see [AI Providers](/en/studio/ai-providers)).
...
A Git-backed agent also shows a **branch selector** and a **sandbox status** indicator while its environment boots.
...
Studio chats run with **Decopilot**. When an agent is backed by code, Decopilot
uses the agent's managed **agent sandbox** for file, shell, git, and preview
operations. There is no runtime or harness choice to configure.
