...
- **Left — main view.** This is the view selected from the sidebar or configured as the project's default. A view can still have its own contextual tabs — for example Preview, Content, and Code inside Site Editor — and temporary output from a chat can appear here without becoming permanent sidebar navigation.
- **Right — chat.** Talk to the agent in natural language; it uses its attached tools to do the work. Your past messages and threads with this agent live here. Above the input are the **model-tier picker** (Fast, Smart, Thinking — see [AI Providers](/en/studio/ai-providers)) and the **runtime picker** (see [Where an agent runs](#where-an-agent-runs)).
...
When the agent is running with a local code-editor runtime, **Open in VSCode** and **Open in Cursor** buttons appear in the Code view's topbar for one-click access to the repo on your machine.
...
- **Decopilot** — _"Runs in an agent sandbox."_ Hosted Studio owns the runtime and, whenever a sandbox is needed, uses its managed AgentSandbox; callers do not choose a sandbox provider. This is the default, with nothing to install.
...
Connections live in **Settings → Connections** at the org level. Inside a project's **Settings** destination, you choose which connections (and which specific tools from each) the agent can use. Pick narrow. An agent with three well-chosen tools behaves more predictably than one with thirty.
