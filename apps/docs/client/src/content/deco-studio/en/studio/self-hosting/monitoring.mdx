...
- **Studio fails to start with `DUCKDB_THREADS must be a positive integer`:** `DUCKDB_THREADS` is validated at startup — a malformed value (`abc`, `0`, `-1`, `1.5`) now fails fast with this error instead of silently becoming `NaN` and surfacing later as an opaque DuckDB failure. Set it to a positive integer (e.g. `2`) or unset it to use all CPUs.
...
| `DUCKDB_THREADS` | *(all CPUs)* | Thread count for the embedded DuckDB engine. Fewer threads lowers peak memory. Must be a positive integer — Studio validates it at startup and fails fast with a clear error otherwise |
