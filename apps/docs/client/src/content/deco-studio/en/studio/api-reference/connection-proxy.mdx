...
The **Connection Proxy** endpoint enables MCP clients (like Cursor, Claude Desktop, or custom applications) to connect directly to a specific upstream MCP server through deco Studio. This endpoint acts as a secure, observable proxy that handles authentication, authorization, credential management, and monitoring.

## Endpoint

The canonical path is org-scoped:
...
<Callout type="warning">
  The legacy unscoped form `POST /mcp/:connectionId` still works but emits a deprecation log on every request and will be removed. Update clients to the `/api/:org/...` path.
</Callout>
...
|-----------|------|----------|-------------|
| `org` | `string` | Required | Organization slug — immutable for the lifetime of the org |
...
Build custom clients that need access to a specific MCP server's tools without going through an agent:
...
|----------|----------|-----------------|
| `/api/:org/mcp/:connectionId` | Direct access to one upstream MCP | All tools from that connection only |
| `/api/:org/mcp/gateway/:virtualMcpId` | Curated tool surface for an agent (a.k.a. virtual MCP) | Tools from multiple connections, composed |
| `/api/:org/mcp/self` | Management operations | deco Studio management tools only |

<Callout type="tip">
  For most production use cases, use **Agents** (`/api/:org/mcp/gateway/:virtualMcpId`) instead of direct connection proxies. Agents give you control over which tools are exposed and let you compose tools from multiple connections behind one endpoint.
</Callout>

## Related

- [**Connections**](/en/studio/connections) - Understanding the Connection abstraction
- [**Agents**](/en/studio/agents) - Compose tools from multiple connections into a single endpoint
