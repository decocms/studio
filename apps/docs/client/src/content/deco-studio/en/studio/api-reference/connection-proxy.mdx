...
| `/mcp/gateway/:agentId` | Curated tool surface for an agent | Tools from multiple connections, composed |
| `/mcp/self` | Management operations | deco Studio management tools only |

<Callout type="tip">
  For most production use cases, use **Agents** (`/mcp/gateway/:agentId`) instead of direct connection proxies. Agents give you control over which tools are exposed and let you compose tools from multiple connections behind one endpoint.
</Callout>
