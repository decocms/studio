---
title: Automações
description: Execuções autônomas de agentes IA disparadas por schedules ou eventos
icon: Zap
---

## O Que São Automações?

Automações são **execuções autônomas de agentes** — agentes de IA que executam em um schedule ou em resposta a eventos, sem intervenção humana. Cada automação configura:
...
- **Gatilhos** — quando a automação dispara (schedule cron, evento de conexão ou manual)
...
Gatilhos cron executam a automação em um schedule recorrente usando sintaxe padrão de expressão cron. Todos os horários são em **UTC**.
...
**Schedule à prova de falhas**: O sistema registra `last_run_at` para cada gatilho. Se o servidor reiniciar, execuções perdidas são detectadas e disparadas no próximo ciclo de verificação (a cada 30 segundos).
