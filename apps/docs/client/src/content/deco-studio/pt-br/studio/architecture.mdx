...
O app desktop nativo envolve a mesma superfície web e adiciona sessões de
terminal locais com escopo de conta. Esses terminais são separados da execução
hosted do chat; eles não são um runtime de chat nem um provider de sandbox.
...
O run executa in-process no worker com o **Decopilot** e usa um **agent sandbox**
gerenciado quando a execução de código é necessária.
...
| **Daemon API** (`/_sandbox/*`) | Bearer `DAEMON_TOKEN` | Superfície de controle: operações de fs (read/write/edit/bash/grep), git (status/diff/publish), exec de scripts, setup (clone → install → start), tasks e eventos SSE. | O cluster (worker para as tools de fs/git/bash do agent; API para setup + eventos da UI) |
...
O sandbox gerenciado usa um sidecar privilegiado para montar o filesystem e
propagá-lo para o container principal; o daemon sem privilégios não consegue
montá-lo diretamente.
...
- **Os agent sandboxes** expõem um preview público e uma control API protegida por token no Kubernetes.
