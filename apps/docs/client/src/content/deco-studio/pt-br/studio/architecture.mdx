...
Client ─▶ CF ─┬─▶ NLB ─▶ Web(nginx) ─▶ API ──┬─▶ MCP Proxy ─▶ Downstream MCP (ext)
              ...
              │                               ├─▶ DB (Postgres)
              │                               ├─▶ NATS
              │                               ├─▶ Sandbox controller (mTLS) ─▶ SandboxClaims
              │                               └─▶ Worker ─┬─▶ LLM
...
| Componente | Função |
| --- | --- |
| **DB** (PostgreSQL, via Kysely) | Sistema de registro: orgs, connections, credential vault, audit, threads + mensagens e `sandbox_runner_state` (escrito apenas pelo sandbox controller). Também mantém as **queues do DBOS e o journal `workflow_status`** que tornam os runs duráveis e recuperáveis. |
...
Os dois caminhos são hosted e passam pelo **sandbox controller** quando precisam de uma sandbox:
...
O Studio não cria sandboxes hosted por conta própria. A API e os workers chamam o **sandbox controller**, um serviço Go separado no namespace das sandboxes, por uma claim API com mTLS. O controller detém o RBAC do Kubernetes, cria os `SandboxClaim`s, é dono do `sandbox_runner_state` e informa onde fica o daemon de cada sandbox e qual bearer o abre. O tráfego do daemon e do preview vai então direto do Studio ou do browser para o pod; nunca passa pelo controller. O controller chama o Studio de volta por mTLS para renovar as credenciais de clone e de org-fs. Ele nunca deve ser acessível pela internet: quem tem um certificado emitido pela CA dele pode iniciar sandboxes com variáveis de ambiente arbitrárias. Em uma única máquina sem Kubernetes, o mesmo controller serve sandboxes como containers Docker (o runtime `docker`).
...
| --- | --- | --- |
| Onde | sandbox controller + operador agent-sandbox: um pod `SandboxClaim` por (user, projectRef) | O `local-api` e o `SandboxManager` da app nativa, com um worktree local por handle |
| Acessado via | o endereço do daemon informado pelo controller: o Service in-cluster (controle); ingress (preview) | interceptação em loopback; `<handle>.localhost:<port>` (preview) |
| Dono do roteamento | A API hosted do Studio sempre passa pelo sandbox controller e reporta `agent-sandbox` | A app nativa intercepta localmente as mesmas chamadas de lifecycle/filesystem e reporta `local-api` |
...
O roteamento do provider segue a superfície que recebe o request, e não uma opção do request: chamadas de lifecycle do browser chegam à API hosted e ao sandbox controller; as mesmas chamadas feitas pela webview nativa são interceptadas pelo `local-api` e operam na sandbox `local-api`. Assim, os registros hosted e desktop continuam distinguíveis sem expor um seletor de provider aos callers.
...
- **O roteamento de sandbox pertence à superfície**: o Studio hosted passa pelo sandbox controller; a app nativa intercepta localmente e registra `local-api`.
- **O Studio não tem acesso ao cluster para sandboxes**: o controller cria os claims e é dono do estado deles.
