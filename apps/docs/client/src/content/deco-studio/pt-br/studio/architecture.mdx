...
<Callout type="warning">
  O recurso de pub/sub de CloudEvents foi removido, incluindo o worker, a fila de entrega e as ferramentas `EVENT_PUBLISH`/`EVENT_SUBSCRIBE`. O contrato de subscriber `ON_EVENTS` permanece. O NATS continua dando suporte a streaming, broadcasts e estado compartilhado; o hub SSE permanece em `apps/api/src/event-bus/`.
</Callout>
...
- **O NATS dá suporte a streaming e coordenação entre pods.** O recurso de pub/sub de CloudEvents foi removido.
