...
- **Studio falha ao iniciar com `DUCKDB_THREADS must be a positive integer`:** `DUCKDB_THREADS` agora é validada no startup — um valor malformado (`abc`, `0`, `-1`, `1.5`) falha rápido com esse erro em vez de virar `NaN` silenciosamente e só aparecer depois como uma falha obscura do DuckDB. Defina um número inteiro positivo (ex. `2`) ou não defina a variável para usar todas as CPUs.
...
| `DUCKDB_THREADS` | *(todas as CPUs)* | Número de threads do engine DuckDB embutido. Menos threads reduz o pico de memória. Deve ser um número inteiro positivo — o Studio valida isso no startup e falha rápido com um erro claro caso contrário |
