...
- **`docker-compose.dev.yml`** — **desenvolvimento**: só as dependências (PostgreSQL + um cluster NATS de 3 nós, topologia prod-like); você roda o Studio **do código-fonte** com `bun run --cwd=apps/mesh dev:server`. Sem sandbox aqui — isso precisa de Kubernetes (veja o loop [dev-hybrid](https://github.com/decocms/studio/tree/main/selfhost/examples/dev-hybrid)).
