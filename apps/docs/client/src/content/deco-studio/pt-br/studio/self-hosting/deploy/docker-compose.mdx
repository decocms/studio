...
- **`docker-compose.dev.yml`** — **desenvolvimento**: só as dependências (PostgreSQL + um cluster NATS de 3 nós, topologia prod-like); você roda os apps de API e web **do código-fonte** com `bun run dev:servers`. O `dev:servers` não inicia um sandbox controller, então sandboxes hospedados precisam de um configurado via `STUDIO_SANDBOX_CONTROLLER_*`. O `bun run dev` inicia um por conta própria no runtime Docker do controller quando `STUDIO_AGENT_SANDBOX_ENABLED=true` e nenhuma URL de controller está definida; para sandboxes no Kubernetes, veja o loop [dev-hybrid](https://github.com/decocms/studio/tree/main/selfhost/examples/dev-hybrid).
