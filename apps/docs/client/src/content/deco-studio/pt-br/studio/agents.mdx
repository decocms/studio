...
Abaixo do preview em tempo real, uma **gaveta de terminal** transmite os logs de setup e dos scripts do sandbox (clone, install, saída do dev server). Arraste a borda superior para redimensioná-la, e use **Show terminal** / **Hide terminal** no menu de três pontos (⋯) da barra de abas para alternar sua visibilidade para o agent atual — essa escolha é lembrada por agent. Para que ela abra automaticamente sempre, em vez de ativá-la por agent, ative **Show preview terminal by default** em **Settings → Account → Profile & Preferences**.
