...
<Callout type="tip">
  Um sandbox na nuvem é compartilhado por branch entre todos na sua org autorizados a trabalhar naquele agent — se um colega abrir o mesmo agent e a mesma branch, vocês dois estarão trabalhando contra o mesmo ambiente e os mesmos arquivos em execução. O runtime local de cada colega (via `deco link`) permanece restrito à própria máquina.
</Callout>
