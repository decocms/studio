...
<Callout type="note">
  **A primeira instalação contra um banco vazio migra antes de servir.** O Studio pega um advisory lock do Postgres em volta da migração do schema de sistema do DBOS, então quem chegar primeiro migra e os outros esperam — não é preciso ajustar contagem de réplicas, com ou sem Argo CD. O chart também traz um Job de migração com escritor único (`migrateJob.enabled`, ligado por padrão) que dá a essa migração fria um lugar para terminar: ele não tem liveness probe, enquanto um container de API que migra durante o boot é morto em cerca de 60 segundos e uma migração fria leva minutos. No `helm install` puro o Job sobe junto com os primeiros pods, então espere alguns restarts desses pods na primeira instalação. Converge sozinho e nada é corrompido. No Argo CD o sync wave segura os Deployments até o Job terminar, e você nem vê os restarts.
</Callout>
