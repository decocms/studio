...
<Callout type="warning">
  **Na primeira instalação contra um banco vazio com Helm puro, instale sem pods da aplicação e só depois ligue** — `--set replicaCount=0 --set worker.replicaCount=0 --set worker.autoscaling.enabled=false`, e então `helm upgrade` com os values reais. Os três são necessários: um pod roda dois containers de API (`api-0`, `api-1`), então `replicaCount: 1` já são dois processos, e `worker.replicaCount: 0` é inócuo enquanto o HPA do worker estiver com `minReplicas: 2`. Todo processo migra o schema de sistema do DBOS no boot, e o SDK do DBOS atualiza a versão do schema com um `UPDATE` sem `WHERE`, assumindo que `dbos.dbos_migrations` tem uma única linha. Duas réplicas chegando nesse ponto juntas deixam duas linhas, e a partir daí todo update colide na primary key e a versão nunca mais avança — os pods entram em `CrashLoopBackOff` permanente e só um `DELETE` manual recupera o banco. O chart traz um Job de migração com escritor único (`migrateJob.enabled`, ligado por padrão) que fecha isso no Argo CD, que segura os Deployments até o Job terminar; no `helm install` puro o Job sobe junto com os primeiros pods, então quem protege é a contagem de réplicas. Upgrades são seguros em qualquer contagem.
</Callout>
