...
### Node placeholder (capacidade de node aquecida)

O `warmPool` pré-aquece *pods* de sandbox, mas cada pod do warm pool ainda precisa de um *node*. Ao reabastecer o pool após um pico, em um scale-up do KEDA/HPA, ou em um claim frio além do pool, os novos pods podem ficar `Pending` enquanto o autoscaler do cluster provisiona um node — o que adiciona latência de claim que o `warmPool` isolado não elimina.

O `nodePlaceholder` (desabilitado por padrão) executa pods "balão" de baixa prioridade que reservam capacidade extra em nodes já em execução:
...
Um pod de sandbox real (prioridade `0`) faz preempção de um balão instantaneamente e ocupa o slot liberado no node aquecido, enquanto o autoscaler reprovisiona um node para o balão removido em segundo plano — assim o claim visível para o usuário nunca espera pelo provisionamento de um node.

<Callout type="warning">
  `priorityClassName` precisa referenciar uma `PriorityClass` com valor abaixo de `0` para que os pods reais de sandbox e do warm pool sempre façam preempção do balão. Quem faz self-hosting sem uma classe de baixa prioridade existente precisa criar uma e configurá-la aqui.
</Callout>

Se não configurados, o `nodeSelector` e as `tolerations` do balão usam por padrão os do próprio pod de sandbox (`values.nodeSelector` / `values.tolerations`), de modo que ele reserva capacidade no mesmo `NodePool` isolado por taint usado pelos sandboxes reais. Dimensione `replicas` de acordo com o pico de claims frios simultâneos que você quer absorver sem espera por provisionamento de node — produção e staging compartilham o mesmo `NodePool`, então dimensione uma vez entre os ambientes, não por ambiente.
...
| `nodePlaceholder.enabled` / `nodePlaceholder.replicas` | Pods "balão" de baixa prioridade que reservam capacidade de node aquecida no NodePool do sandbox | `false` / `2` |
