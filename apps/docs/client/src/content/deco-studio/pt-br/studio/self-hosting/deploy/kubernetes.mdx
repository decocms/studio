...
## Sandbox (execução de código & previews)

O sandbox **não faz parte deste chart** — são dois charts separados instalados em `agent-sandbox-system`: `sandbox-operator` (CRDs + controller, cluster-wide) e `sandbox-env` (template por-env, RBAC, warm pool). Para as claims de sandbox funcionarem, o Studio e o `sandbox-env` precisam concordar em três coisas — os bloqueios comuns de self-host:

- `serviceAccount.create: true` no Studio (ele roda com a ServiceAccount que o RBAC do sandbox concede; rodar como `default` → `403 cannot create sandboxclaims`).
- um **sentinel token compartilhado** — o mesmo valor em `sandbox-env.sentinel.token` e no `STUDIO_SANDBOX_SENTINEL_TOKEN` do Studio (senão o mesh faz cold-provision e o template rejeita `DAEMON_TOKEN`).
- `STUDIO_SANDBOX_PROVIDER=agent-sandbox` + `STUDIO_ENV=<env>` + `STUDIO_SANDBOX_TEMPLATE_NAME=studio-sandbox-<env>`.

Como uma instalação completa envolve múltiplos charts que compartilham valores, recomenda-se envolvê-los no seu próprio umbrella chart. Veja [`selfhost/production`](https://github.com/decocms/studio/tree/main/selfhost/production) para um exemplo endurecido e [`selfhost/examples/k8s-local`](https://github.com/decocms/studio/tree/main/selfhost/examples/k8s-local) para uma configuração local completa e funcional.
