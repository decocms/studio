...
const NODES: Record<string, Node> = {
  ...
  llm: {
    ...
    role: "Model provider",
    desc: "Model providers called by hosted Decopilot, hosted Claude Code inside AgentSandbox, or a native coding CLI. Hosted credentials are injected per run; native CLIs use the user's local authentication.",
    facts: [
      ["Models", "Claude"],
      ["Called by", "hosted / native harness"],
    ],
  },
  dmcp: {
    x: 1004,
  ...
  nats: {
    ...
    desc: "Live infra (not the dormant event-bus feature). Holds the fenced JetStream run log consumed by /stream and the durable projector, carries cross-pod cancellation, and stores shared MCP-list cache and connection-circuit state in JetStream KV.",
    facts: [
      ["Run log", "/stream + projector"],
      ["Core NATS", "cross-pod cancel"],
      ["KV", "MCP cache + circuits"],
    ],
  },
  db: {
    x: 930,
  ...
  localapi: {
    ...
    label: "local-api",
    tag: "axum · embedded",
    role: "Native control API",
    desc: "Embedded in the Tauri app on one authenticated loopback origin. Proxies shared data and MCP traffic to Studio, but intercepts native-only thread, harness, sandbox, filesystem, Git, terminal, task, and preview routes locally.",
    facts: [
      ["Origin", "authenticated loopback"],
      ["Upstream", "Studio API"],
      ["Owns", "native-only routes"],
    ],
  },
  nativecli: {
    x: 1300,
    ...
    tag: "pty · local cli",
    role: "Desktop coding agent",
    desc: "The user's Claude Code, Codex, or OpenCode CLI, launched by local-api in the thread's local worktree. Provider authentication stays on the machine; scoped MCP reaches Studio through the local API's upstream proxy.",
    facts: [
      ["Harnesses", "Claude · Codex · OpenCode"],
      ["Process", "interactive PTY"],
      ["MCP", "local-api → Studio"],
    ],
  },
  dsbx: {
    x: 1290,
    ...
    desc: "A SandboxManager-owned local worktree and setup runtime, one per handle. The native webview's lifecycle and filesystem calls terminate in local-api, which reports local-api; preview is served through <handle>.localhost:<port>.",
    facts: [
      ["Runs on", "laptop"],
      ["Preview", "handle.localhost"],
      ["Control", "local-api interception"],
    ],
  },
  orgfsd: {
    x: 1460,
    ...
    tag: "mount · daemon",
    role: "Org filesystem mount",
    desc: "The org filesystem mounted on the laptop at <appRoot>/org/<volume>. No sidecar — local-api resolves and maintains the mount directly. Backend: it calls Studio's /api/:org/fs/* routes (the same S3 store as the cloud sandbox).",
    facts: [
      ["Mount", "rclone → WebDAV"],
      ["Desktop", "local-api, no sidecar"],
      ["Backend", "/api/:org/fs → S3"],
    ],
  },
};
