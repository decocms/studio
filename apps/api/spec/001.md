...
function getDatabaseUrl(): string {
  return process.env.DATABASE_URL || './data/studio.db';
}
...
const result = await auth.api.createApiKey({
  body: {
    ...
    permissions: {
      ...
      // These tools manage Studio itself (connections, etc.)
...
app.post('/mcp/:connectionId', async (c) => {
  ...
  if (!session) {
    return new Response(null, {
      status: 401,
      headers: {
        'WWW-Authenticate': 'Bearer realm="Studio"'
      }
    });
  }
...
export const tracer = trace.getTracer('studio', '1.0.0');
export const meter = metrics.getMeter('studio', '1.0.0');
...
```typescript
// 1. AUTHORIZATION SERVER (for MCP clients connecting to Studio)
...
// 2. RESOURCE SERVER (for Studio's own tools)
...
```typescript
// Example: Claude Desktop or any MCP client connecting to Studio

import { Client } from '@modelcontextprotocol/sdk/client/index.js';
import { StreamableHTTPClientTransport } from '@modelcontextprotocol/sdk/client/streamableHttp.js';
import { createRemoteJWKSet, jwtVerify } from 'jose';

const STUDIO_URL = 'https://studio.example.com/mcp';

async function connectToStudio() {
  // Step 1: Discover OAuth metadata
  const resourceMetadata = await fetch(`${STUDIO_URL}/.well-known/oauth-protected-resource`)
    .then(r => r.json());
  ...
  authUrl.searchParams.set('resource', STUDIO_URL); // RFC 8707 - Audience binding
...
// Studio automatically detects that connections providing these tools
...
// Usage
const db = createDatabase(process.env.DATABASE_URL || 'file:./studio.db');
```
...
app.post('/:connectionId', async (c) => {
  ...
  const client = new Client({ name: 'studio-proxy', version: '1.0.0' });
  const transport = new StreamableHTTPClientTransport(
...
- Create a local SQLite database at `./data/studio.db`
...
const databaseUrl = process.env.DATABASE_URL || "file:./data/studio.db";
...
interface StudioTokenPayload {
  ...
  iss: string;           // Issuer: Studio instance base URL (e.g., "https://studio.example.com")
...
interface StudioTokenPayload {
  // Standard JWT claims
  sub: string;           // Subject: Token ID or User ID
  iss: string;           // Issuer: Studio base URL
...
curl -L https://github.com/your-org/studio/releases/latest/download/studio-linux -o studio
chmod +x studio
...
# SQLite database is automatically created at ./data/studio.db
```
...
```bash
# Database (optional - defaults to SQLite at ./data/studio.db)
DATABASE_URL=postgresql://user:pass@host:5432/dbname
```
...
2. **Database Backups**:
   - SQLite: Simple file-based backups (`cp data/studio.db backups/studio-$(date +%Y%m%d).db`)
