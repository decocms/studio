...
Extend the plugin system to support root-level public routes, then create a Studio plugin (`studio-plugin-monitoring-share`) that enables sharing read-only, data-scoped monitoring dashboards with external clients via presigned URLs.
...
**2. Update main app** in `apps/web/src/index.tsx`:
...
**3. Migrate user-sandbox** - After this change, the `connectRoute` in `apps/web/src/routes/connect.tsx` can be moved into the user-sandbox plugin itself, removing the workaround.
...
```
packages/studio-plugin-monitoring-share/
├── package.json
...
- [ ] Update `apps/web/src/index.tsx` to pass rootRoute and collect public routes
- [ ] Move connect route into user-sandbox plugin using new API
- [ ] Remove/deprecate `apps/web/src/routes/connect.tsx` workaround

### Phase 2: Plugin Package
- [ ] Create package structure for `studio-plugin-monitoring-share`
...
| `apps/web/src/index.tsx` | Pass new context props, collect and mount public routes |
| `packages/@decocms/sandbox/client/index.ts` | Migrate connect route registration |
| `apps/web/src/routes/connect.tsx` | Remove (or keep as fallback) |

### Phase 2 & 3 (Plugin)
| File | Action |
|------|--------|
| `packages/studio-plugin-monitoring-share/*` | Create entire plugin package |
| `apps/web/src/plugins.ts` | Register client plugin |
| `apps/api/src/api/plugins.ts` | Register server plugin |
| `apps/web/src/routes/orgs/monitoring.tsx` | Add Share button |
