 ...
 * in the `Session` shape `apps/api/src/cli/lib/session.ts` defines.
 *
 ...
 *
 * This helper is called only by the local development boot path.
 */
...
function devLinkSessionPath(homeDir: string): string {
  // `DEV_LINK_SESSION_PATH` is set by the dev CLI (`apps/api/src/cli/commands/dev.ts`)
...
export async function bootstrapDevLinkSession(
  ...
  // stable `<username>@localhost.studio` email (with a legacy-domain lookup
  // for existing data). Pinning to the email — rather
