...
/**
 * The values the template interpolates. Pure, so the branch selection — which
 * repo block, which re-run lead — stays unit-tested now that the prose around
 * it is org-editable.
 */
export function taskInitialPromptVars(
  task: {
    id: string;
    title: string;
    description: string | null;
  ...
  return {
    ...
    jiraUrl: task.externalUrl ?? "",
    repoContext: repoContextBlock(repo, opts?.repoChoices),
    prBullet:
      "- Make the change, commit it, push the branch, and open a pull request" +
      (opts?.pr
        ? " — or push to the existing one, per the instruction above."
        : "."),
    prContext: prContextBlock(opts),
  };
}

/** The description, with uploads rewritten to sandbox paths. "" when absent. */
function descriptionBlock(description: string | null): string {
  if (!description) return "";
  const rewritten = uploadsAsSandboxPaths(description);
  const hint = sandboxUploadHint(description, rewritten);
  return ["Description:", rewritten, ...(hint ? ["", hint] : [])].join("\n");
}

/**
 * Where the code is. `repo: null` is the several-repos case: the working
 * directory is EMPTY and the first instruction is to clone one. That has to be
 * stated up front and unambiguously — a model that starts by looking for files
 * it was told exist spends its first steps concluding the sandbox is broken.
 */
function repoContextBlock(
  repo: TaskRepo | null,
  repoChoices: TaskRepoChoiceOption[] | undefined,
): string {
  if (repo) {
    return `The repository ${repo.owner}/${repo.name} is already cloned at your working directory, on its own branch. \`git\` and \`gh\` are authenticated. ${SHALLOW_CHECKOUT_NOTE}`;
  }
  return [
...
/**
 * Why this is a re-run. Its whole point is to update the EXISTING pull request,
 * not to redo the task or open a second one. Conflict resolution wins over
 * feedback. "" on a first attempt.
 */
function prContextBlock(opts: SuperAgentPromptOpts | undefined): string {
  ...
  if (opts?.pr) {
    ...
    // request" in the template would contradict where the run actually is.
    return [
