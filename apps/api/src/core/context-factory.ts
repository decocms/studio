...
import { verifyStudioToken } from "../auth/jwt";
import { CredentialVault } from "../encryption/credential-vault";
import { getSettings } from "../settings";
import { getBaseUrl } from "./server-constants";
import { ConnectionStorage } from "../storage/connection";
import { CommerceDiscoveryReportStorage } from "../storage/commerce-discovery-reports";
...
export function createBoundAuthClient(ctx: AuthContext): BoundAuthClient {
  ...
  return {
    ...
    hasPermission: async (
      ...
      // connection grants, and nothing else. See ADMIN_ROLES in
      // @decocms/shared/auth/roles.
...
import { BUILTIN_ROLES, hasAdminRole } from "@decocms/shared/auth/roles";
 ...
 */
// Tiny TTL cache for org archived-status, keyed by org id. The Studio JWT
...
// Read-through member-role cache; shared by the Studio JWT and API-key auth paths.
 ...
 * creator) but ALSO forwards the REAL acting user in an `x-studio-token` JWT (see
 ...
 *
 * `x-studio-token` is only ever set by Studio's outbound header builder, so an
...
async function resolveOnBehalfOfUser(
  ...
  const studioToken = readStudioHeader(req.headers, "token");
  if (!studioToken) return undefined;

  const payload = await timings.measure("auth_verify_onbehalf_studio_jwt", () =>
...
async function authenticateRequest(
  ...
  if (authHeader?.startsWith("Bearer ")) {
    ...
    try {
      const studioTokenPayload = await timings.measure(
        "auth_verify_studio_token",
        () => verifyStudioToken(token),
      );

      if (studioTokenPayload) {
        // Look up user's organization role for admin/owner bypass
        let role: string | undefined;
        const organizationId = studioTokenPayload.metadata?.organizationId;
        if (studioTokenPayload.sub && organizationId) {
          role = await resolveAndCacheRole(
            db,
            timings,
            studioTokenPayload.sub,
            organizationId,
            memberRoleCache,
          );
        }
        ...
        const metaOrgId = studioTokenPayload.metadata?.organizationId;
        if (metaOrgId) {
          const metaName = studioTokenPayload.metadata?.organizationName;
          const metaSlug = studioTokenPayload.metadata?.organizationSlug;
          if (metaName || metaSlug) {
            ...
            if (
              await isOrgArchivedCached(
                ...
                timings,
                metaOrgId,
                "auth_query_org_archived_for_studio_token",
              )
            ) {
            ...
            const orgRow = await timings.measure(
              "auth_query_org_for_studio_token",
              () =>
                db
                  .selectFrom("organization")
        ...
        return {
          ...
          role,
          permissions: studioTokenPayload.permissions,
    ...
    try {
      ...
      if (result?.valid && result.key) {
        ...
        // Studio token header (including its legacy alias). Credit that user
        // (attribution) while keeping the API key's permissions as the
...
export async function createStudioContextFactory(
  ...
  const baseStorage = {
    ...
    commerceDiscoveryReports: new CommerceDiscoveryReportStorage(
      config.db,
      vault,
    ),
  ...
  return async (
    ...
    // Must NOT be a singleton — per-request Studio token headers get baked into
    // the transport at creation time and would go stale across requests.
