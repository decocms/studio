 ...
 * reports's `share_my_diagnostic` tool.
 *
 ...
 * preview of the private diagnostic — sent by reports (it owns the
 ...
 * URL; reports sends the preview email with that URL as its CTA.
 *
 * The accept URL's `redirectTo` is the deep link that OPENS the diagnostic app
 * view — the exact shape `reportNavTarget()`
 * (web/hooks/use-reports-diagnostic.ts) and setup.ts's completion-email link
 ...
 *  Mirrors reportNavTarget() and setup.ts's completion-email link.
...
export function diagnosticDeepLinkPath(orgSlug: string, orgId: string): string {
  return agentPanelPath(orgSlug, {
    projectId: getReportsAgentId(orgId),
    panel: "app",
    search: {
      connection: WellKnownOrgMCPId.REPORTS(orgId),
      tool: REPORTS_TOOL_NAME,
    },
  });
}

export const createReportsShareRoutes = () => {
  ...
  app.post("/internal/commerce-diagnostic/share-invite", async (c) => {
    ...
    // Names no thread, like reportNavTarget: the chat opens an empty composer.
