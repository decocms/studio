...
const ALWAYS_INCLUDE = [
  ...
  //
  // Force-include every `@opentelemetry/*` package that apps/api declares
  ...
  // (node_modules/.bun/...) doesn't surface them at apps/api's level,
  ...
  //
  // If you add a NEW direct `@opentelemetry/*` import to apps/api, add it
  // both to apps/api/package.json AND to this list.
  "@opentelemetry/api",
...
// Script is at apps/api/scripts, so we need to go up three levels to the repo root
const WORKSPACE_ROOT = resolve(SCRIPT_DIR, "../../..");
const API_APP_ROOT = resolve(SCRIPT_DIR, "..");
...
async function copyPackage(
  ...
  try {
    ...
    // through symlinks (apps/api/node_modules/@opentelemetry/api →
...
async function pruneNodeModules(): Promise<Set<string>> {
  ...
  for (const entryPoint of ALWAYS_INCLUDE) {
    try {
      const resolved = Bun.resolveSync(entryPoint, API_APP_ROOT);
...
/**
 * Reads apps/api/package.json's runtime `dependencies` set. Anything listed
...
async function readConsumerInstalledDeps(): Promise<Set<string>> {
  const pkgJson = JSON.parse(
    await readFile(join(API_APP_ROOT, "package.json"), "utf-8"),
  ) as {
 ...
 *   - All apps/api OTel devDeps go through nft's known-blindspot path
