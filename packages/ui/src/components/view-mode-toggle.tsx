...
export function ViewModeToggle<T extends string = string>({
  ...
  return (
    <div
      className={cn(
        "relative flex gap-0 bg-muted classic:rounded-lg compact:rounded-full",
        className,
      )}
    >
      {options.map((option, i) => {
        const btn = (
          <button
            ...
            className={cn(
              "relative z-10 flex items-center justify-center gap-2 classic:rounded-lg compact:rounded-full transition-colors [transition-timing-function:var(--ease-out-cubic)] duration-200",
              fullWidth
                ? "flex-1 h-12 px-4"
                : option.label
      ...
      <div
        className={cn(
          "absolute z-0 bg-background classic:rounded-lg compact:rounded-full border-shadow transition-all [transition-timing-function:var(--ease-out-cubic)] duration-200",
