...
export const ThreadCreateDataSchema = z.object({
  ...
  branch: z
    ...
    .optional()
    .describe(
      "Preferred branch. Used only when the vMCP has a githubRepo; ignored otherwise. When omitted, the server picks the most-recently-touched AgentSandbox branch from the user's sandboxMap, falling back to a freshly generated name.",
    ),
});
