  ...
  try {
    if (process.platform === "darwin") {
      const token = execSync(
        `security find-generic-password -a "deco-mesh" -s ${JSON.stringify(meshUrl)} -w 2>/dev/null`,
        { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] },
      ).trim();
      if (token) return token;
    } else if (process.platform === "linux") {
      const token = execSync(
        `secret-tool lookup service "deco-mesh" url ${JSON.stringify(meshUrl)} 2>/dev/null`,
        { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] },
      ).trim();
      if (token) return token;
    }
...
export async function saveMeshToken(
  ...
  try {
    if (process.platform === "darwin") {
      execSync(
        `security add-generic-password -a "deco-mesh" -s ${JSON.stringify(meshUrl)} -w ${JSON.stringify(apiKey)} -U`,
      ...
      execSync(
        `printf '%s' ${JSON.stringify(apiKey)} | secret-tool store --label "deco-mesh" service "deco-mesh" url ${JSON.stringify(meshUrl)}`,
        { stdio: ["pipe", "pipe", "pipe"] },
      );
      savedToKeychain = true;
...
/**
 * Create a persistent API key via Better Auth's API key endpoint.
 *
...
async function createMeshApiKey(
  ...
  const res = await fetch(`${meshUrl}/api/auth/api-key/create`, {
    method: "POST",
    headers: {
  ...
  if (!key) {
    throw new Error(
      "Unexpected response from /api/auth/api-key/create — no key field",
    );
  }
  return key;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = createServer(async (req, res) => {
      try {
        ...
        if (tokenParam) {
          // Token passed directly in URL (simpler callback strategy)
          apiKey = await createMeshApiKey(meshUrl, "");
          // In this case use the token as the session bearer
