...
// POST /_sandbox/config and GET /health. It is the daemon's only exported
// package, so the sandbox controller compiles against the same definitions
...
type GitRepository struct {
	...
	// Credentials for private submodules, keyed by host. A nil slice is absent
	// (keep current); an empty one is sent as `[]` and clears them, which is how
	// a revoked PAT leaves a reused pod. `omitzero`, not `omitempty`: the
	// latter drops the empty slice too.
	SubmoduleCredentials []SubmoduleCredential `json:"submoduleCredentials,omitzero"`
}
...
type GitConfig struct {
	...
	// field-wise merge of a list has no sane answer for "which entry". Nil is
	// absent; an empty slice is sent and drops them all (see
	// SubmoduleCredentials for why `omitzero`).
	Repositories []GitRepository `json:"repositories,omitzero"`
...
type ConfigAuth struct {
	// RotateToken replaces the daemon's bearer, authorized by the token on the
	// request, and is applied BEFORE the patch — so it has taken effect even
	// when the patch is then rejected. That is how a warm-pool pod's shared
	// sentinel becomes a per-claim secret, and why a rotating request must
...
// the optional auth block beside it.
type ConfigRequest struct {
	TenantConfig
	Auth *ConfigAuth `json:"auth,omitempty"`
}

// ConfigResponse is POST /_sandbox/config's 200 body. Config has the submodule
// tokens stripped.
...
// SetupState is the boot sequence as /health reports it.
type SetupState struct {
	Running bool `json:"running"`
	Done    bool `json:"done"`
}

// Health is GET /health — unauthenticated, so it is what a caller polls before
// it holds a per-claim token.
type Health struct {
