 ...
 *
 * We use Node's `child_process.spawn` rather than `Bun.spawn` because Bun's
 ...
 *
 * The dev server itself still uses `spawnPty` directly (see task-manager.ts)
 * because it benefits from colored output and progress bars.
 *
 * stdout and stderr are interleaved into the onChunk stream so callers see
 * the same log shape as the previous PTY-backed implementation.
 */
export async function spawnSetupStep(
  cmd: string,
