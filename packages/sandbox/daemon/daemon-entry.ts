...
import { streamViaHarnessRunner } from "./harness-runner/client";
import { DISPATCHABLE_HARNESS_IDS } from "./harness-runner/protocol";
import {
  killHarnessRunnerSync,
  shutdownHarnessRunner,
} from "./harness-runner/supervisor";
...
// loopback when it pulls a work item; the daemon forwards the run to the
// harness-runner subprocess (see harness-runner/) and relays its
// `UIMessageChunk`s back as SSE. The CLI factories live in the runner
// process, not here; the cluster `decopilotHarnessFactory` (RunRegistry,
// run-stream internals, StudioContext) is in neither.
const dispatchableHarnessIds = new Set<string>(DISPATCHABLE_HARNESS_IDS);
const lookupDispatchHarness = (id: string, input: unknown) => {
  if (!dispatchableHarnessIds.has(id)) {
  ...
  return {
    ...
    stream: async function* () {
      ...
      yield* streamViaHarnessRunner(id, harnessInput);
    },
  };
};
