...
export class TaskManager {
  ...
  private startPty(task: TaskInternal): void {
    ...
    try {
      ...
      // node-pty's forkpty can fail deterministically on some hosts (observed:
      // bun + libuv on macOS throwing `posix_spawnp failed`). Fall back to the
      // non-PTY pipe spawn so the dev server still boots; the user loses
      // colored output and progress bars but everything else works.
  ...
  private startPipe(task: TaskInternal): void {
    ...
    // `npm`, etc. Merge process.env in to preserve the shell environment.
