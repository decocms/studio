...
/**
 * Paths generated by build tools, package managers, and framework caches.
 * Changes in these directories are noise for the `file-changed` SSE event —
 * they'd trigger pointless liveMeta/decofile invalidations on the UI.
 * `.git/` is already filtered by the caller before reaching this check.
 */
const WATCHER_NOISY_DIRS = new Set([
  "node_modules",
  ".next",
  "dist",
  "build",
...
function isWatcherPathNoisy(filename: string): boolean {
  const first = filename.split("/")[0];
  return WATCHER_NOISY_DIRS.has(first);
}
      ...
      this.repoWatcher = fs.watch(
        ...
        (_event, filename) => {
          ...
          if (typeof filename === "string") {
            // Skip noisy paths that would trigger pointless SSE events
            // and UI cache invalidations (build output, package installs).
            if (!isWatcherPathNoisy(filename)) {
