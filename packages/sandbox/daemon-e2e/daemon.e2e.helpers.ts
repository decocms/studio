...
const DAEMON_BINARY = join(import.meta.dir, "..", "daemon-go", "bin", "daemon");
 ...
 *
 * `raw` unset/empty: run the Go daemon built at `daemon-go/bin/daemon`
 * (`go build -o bin/daemon .`). Another implementation sets `DAEMON_E2E_CMD`
 * to its own argv — either a JSON array (`["./target/release/daemon"]`,
 * required when args contain spaces) or a plain whitespace-separated string
 * (`"bun /abs/daemon.js"`). Passed in rather than defaulted so the "no
 * override" case stays testable under a DAEMON_E2E_CMD run.
 */
export function resolveDaemonCmd(raw: string | undefined): string[] {
  if (!raw || raw.trim().length === 0) return [DAEMON_BINARY];
  try {
...
export function buildDaemonEnv(
  ...
  return {
    ...
    // real credential store. The sandbox daemon ignores this local-api setting.
