 ...
 * lives on `DockerSandboxProvider`, not on the `SandboxProvider` interface.
 */

import { DockerSandboxProvider, type DockerProviderOptions } from "./runner";
...
export async function sweepDockerOrphansOnBoot(
  ...
  try {
    const runner = new DockerSandboxProvider(opts);
...
export async function sweepDockerOrphansOnShutdown(
  runner: DockerSandboxProvider | null,
): Promise<void> {
  if (!runner) return;
