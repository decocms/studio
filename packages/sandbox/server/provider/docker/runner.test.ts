...
import { DockerSandboxProvider } from "./runner";
import type {
  RunnerStateRecord,
...
describe("DockerSandboxProvider.ensure() — fresh provision", () => {
...
describe("DockerSandboxProvider.ensure() — adopt by label", () => {
...
describe("DockerSandboxProvider.ensure() — --name collision recovery", () => {
...
describe("DockerSandboxProvider.ensure() — in-process dedupe", () => {
...
describe("DockerSandboxProvider.ensure() — resume from persisted state", () => {
  it("uses persisted token/workdir, no docker run, alive + health ok", async () => {
    ...
    const runner = new DockerSandboxProvider({ exec, stateStore: store });
    const sandbox = await runner.ensure(ID);
...
describe("DockerSandboxProvider.sweepOrphans()", () => {
  ...
  it("returns full count even if one stop rejects", async () => {
    ...
    const runner = new DockerSandboxProvider({ exec });
    const n = await runner.sweepOrphans();
    expect(n).toBe(3);
...
describe("DockerSandboxProvider.delete()", () => {
...
describe("DockerSandboxProvider — sanity: preview URL & port resolvers", () => {
