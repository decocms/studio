 ...
 *
 * Isolated in its own file so `daemon-spawn.ts` can `await import()` it
 ...
 * bytes here so no asset has to ship alongside the bundle. The
 * `materializeDaemonBundle` helper writes these bytes to disk on first
 * spawn and points `bun run` at the materialized file — see
 * `daemon-spawn.ts`.
 */

// @ts-expect-error - Bun-specific text loader attribute; TS resolves the
// underlying .js file and doesn't model `with { type: "text" }`.
import _daemonBundle from "../daemon/dist/daemon.js" with { type: "text" };
