# Sandbox controller — taking sandbox provisioning out of Studio

Written 2026-08-14. Status: **proposal, nothing built.**

Studio talks to the Kubernetes API itself to provision sandboxes. This moves
that to a separate service, **written in Go**, so (a) `agent-sandbox`, AWS
Lambda microVMs and a plain container runtime are **live, concurrently
available runtimes** the controller chooses between per sandbox — not a
compile- or deploy-time choice, and not something Studio knows about — and
(b) local dev gets a sandbox backend that needs a container runtime but not a
cluster.

Unqualified TS paths are `packages/sandbox/server/provider/`; unqualified Go
paths are `packages/sandbox/`.
...
- opens `127.0.0.1` port-forwarders through the apiserver when Studio runs
  outside the cluster (kind local dev);
...
**Studio keeps talking to the daemon directly.** In production it already does
— `proxyDaemonRequest` fetches
`http://<sandbox>.<ns>.svc.cluster.local:9000` (`runner.ts:711`) — and preview
traffic already bypasses Studio entirely via per-claim `HTTPRoute`. So the
controller never carries daemon bytes: it returns *where the daemon is and what
token opens it*, and Studio's existing fetch code is unchanged.
...
`POST /api/_admin/sandbox-clone-url { repo, bufferMs } → { cloneUrl }`, hit by
the credential refresher and the warm-pool reconciler. ~30 lines on each side
and the refresh loops keep working unmodified.
...
1. **Contract + `RemoteSandboxProvider`.** Define the HTTP shape, implement the
   TS client in Studio behind `STUDIO_SANDBOX_PROVIDER=remote`. Default stays
   `agent-sandbox` in-process. Nothing in prod changes.
2. **Go controller with the `docker` runtime as its only registered one.** All
...
3. **Port the `agent-sandbox` runtime.** The long pole. Validate against the
...
4. **Staging, then production**, then delete the in-process `agent-sandbox` case
   from `lifecycle.ts`, the `@kubernetes/client-node` dependency, and Studio's
   K8s RBAC. Until that deletion lands this has made things *worse* (two
   implementations) — do not leave it half-done.
5. **Lambda microVM backend**, when someone actually needs it.
...
3. **Preview in the docker backend.** `http://127.0.0.1:<port>` works but loses
   ...
   cookie/CORS problem.
4. **Idle TTL ownership.** Today the operator's `shutdownTime` enforces it and
   Studio's housekeeper sweeps on daemon idle. Under docker the controller
   enforces it in-process. Two implementations of one policy is acceptable; two
   *definitions* of it is not — keep the window a controller-level config.
