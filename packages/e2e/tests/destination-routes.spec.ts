...
test.describe("destination routes", () => {
  ...
  test("a project surface is canonical, and legacy search settles on it", async ({
    ...
    /* Chat sits to the left of Main. Pin both DOM and visual order so
       keyboard traversal agrees with what the user sees. */
    ...
    expect(chatColumnBox.x).toBeLessThan(separatorBox.x);
    expect(separatorBox.x).toBeLessThan(mainColumnBox.x);

    /* Chat toggles from the sidebar header with an icon, tooltip, and state.
       The panel topbar is reserved for the route's own content. */
    await expect(
    ...
    const sidebar = page.locator('[data-slot="sidebar"]');
    await expect(
      mainPanel(page).locator('button[aria-controls="workspace-side-panel"]'),
    ).toHaveCount(0);
    const hideChat = sidebar.getByRole("button", {
      name: "Hide chat",
      exact: true,
    });
    ...
    await expect(hideChat).toHaveAttribute("aria-pressed", "true");
    await expect(hideChat).toHaveText("");
    await hideChat.hover();
    await expect(
      page.getByRole("tooltip", { name: "Hide chat", exact: true }),
    ).toBeVisible();
    await hideChat.click();
    await expect(chatPanel(page)).toBeHidden({ timeout: SHELL_TIMEOUT_MS });
    const showChat = sidebar.getByRole("button", {
      name: "Show chat",
      exact: true,
    });
    await expect(showChat).toHaveText("");
    await expect(showChat).toHaveAttribute("aria-pressed", "false");
    await expect(showChat).toHaveAttribute("aria-expanded", "false");
    await showChat.click();
    await expect(chatPanel(page)).toBeVisible({ timeout: SHELL_TIMEOUT_MS });
    await expect(hideChat).toHaveText("");
    await expect(hideChat).toHaveAttribute("aria-expanded", "true");

    // The same control remains reachable in the collapsed sidebar rail.
    ...
    await mainPanel(page)
      ...
      .hover();
    await showChat.hover();
    await expect(
      page.getByRole("tooltip", { name: "Show chat", exact: true }),
    ).toBeVisible();
    ...
    expect(visibleChatButtons).toEqual(["Chats", "Show panel"]);
    ...
    /* Reopening must restore the same semantic and visual order. A panel
       library can otherwise prepend the restored node before Chat even though
    ...
    expect(reopenedChatBox.x + reopenedChatBox.width).toBeLessThanOrEqual(
      reopenedMainBox.x,
    );

    // Hiding Chat from the sidebar also works when it is the only open panel.
    await page.goto(
      `/${orgSlug}/projects/${projectId}/settings?sidepanel=true&mainpanel=false`,
    );
    await hideChat.click();
  ...
  test("the sidebar chat toggle stays usable while the project and messages load", async ({
    authedPage: { page, orgSlug },
  }) => {
    const request = page.context().request;
    const orgId = await findOrgId(request, orgSlug);
    const agentId = await createProject(request, orgSlug, "loading toggle e2e");
    const threadId = await createThread(request, orgSlug, agentId);
    await populateThread(orgId, threadId, "A chat that is still loading");

    const entityReady = Promise.withResolvers<void>();
    const releaseEntity = Promise.withResolvers<void>();
    const messagesReady = Promise.withResolvers<void>();
    const releaseMessages = Promise.withResolvers<void>();
    const entityUrl = `**/api/${orgSlug}/tools/COLLECTION_VIRTUAL_MCP_GET`;
    const messagesUrl = `**/api/${orgSlug}/tools/COLLECTION_THREAD_MESSAGES_LIST`;
    // Hold real responses at the browser boundary, without inventing payloads.
    await page.route(entityUrl, async (route) => {
      if (route.request().postDataJSON()?.id !== agentId) {
        await route.continue();
        return;
      }
      const response = await route.fetch();
      entityReady.resolve();
      await releaseEntity.promise;
      await route.fulfill({ response });
    });
    await page.route(messagesUrl, async (route) => {
      if (route.request().postDataJSON()?.thread_id !== threadId) {
        await route.continue();
        return;
      }
      const response = await route.fetch();
      messagesReady.resolve();
      await releaseMessages.promise;
      await route.fulfill({ response });
    });

    const path = `/${orgSlug}/projects/${agentId}/settings`;
    const toggle = page
      .locator('[data-slot="sidebar"]')
      .locator('button[aria-controls="workspace-side-panel"]');
    try {
      await page.goto(`${path}?thread=${threadId}&sidepanel=false`);
      await entityReady.promise;
      ...
      if (!original)
        throw new Error("The sidebar toggle must exist during loading");
      ...
      releaseEntity.resolve();
      releaseMessages.resolve();
      await page.unroute(entityUrl);
      await page.unroute(messagesUrl);
    }
  });
  ...
  test("a populated thread reopens Chat on cold load unless search closes it", async ({
    ...
    const sidebarToggle = page.locator(
      '[data-slot="sidebar"] button[aria-controls="workspace-side-panel"]',
    );
    ...
    // A loaded conversation's default belongs to its scope. Keep the same
    // sidebar control while leaving the thread and restore its state on Back.
    const originalToggle = await sidebarToggle.elementHandle();
    if (!originalToggle) throw new Error("The sidebar toggle must be mounted");
    await page
    ...
    await page.waitForURL((url) => url.pathname === `/${orgSlug}/home`);
    await expect(
      mainPanel(page).getByRole("heading", { name: "Home", exact: true }),
    ).toBeVisible();
    await expectChatCollapsed(page);
    ...
    await page.goBack();
    await page.waitForURL((url) => url.searchParams.get("thread") === threadId);
    await expect(chatPanel(page)).toBeVisible({ timeout: SHELL_TIMEOUT_MS });
    await expect(sidebarToggle).toHaveAttribute("aria-pressed", "true");
    expect(
      await originalToggle.evaluate((element) => element.isConnected),
  ...
  test("keyboard tab and sidebar navigation focus the new desktop route heading", async ({
    ...
    await page.waitForURL(
      (url) =>
        url.pathname === `/${orgSlug}/projects/${agentId}/site-editor/content`,
      { timeout: SHELL_TIMEOUT_MS },
    );
    await expect(
      mainTopbarRegion(page, "left").getByRole("heading", {
  ...
  test("workspace history and breakpoints preserve a meaningful focus target", async ({
    ...
    // The sidebar toggle hands focus to the mobile switcher and back.
    await hideChat.focus();
    await page.setViewportSize({ width: 767, height: 720 });
    await expect(mobileViewSelect).toBeFocused();
    await page.setViewportSize({ width: 1280, height: 720 });
