...
test.describe("fast preview publish surface", () => {
  test("both the primary half and the dropdown open a popover that stays open", async ({
    ...
    expect(chatBox.y + chatBox.height).toBeLessThanOrEqual(mainBox.y);
    ...
    await expect(mainPanel.locator('[data-slot="main-toolbar"]')).toHaveCount(
      0,
    );
    for (const name of ["Preview", "Content"]) {
      await expect(
        editorToolbar.getByRole("button", { name, exact: true }),
      ).toBeVisible();
    }
    ...
    /* Wide workspaces keep Chat physically left. A user can still resize Main
    ...
    await page.mouse.move(
      columnMainBox.x + columnMainBox.width - 560,
      separatorY,
      { steps: 10 },
    );
    await page.mouse.up();
    ...
    // The persisted split deliberately gives Chat a substantial left column;
