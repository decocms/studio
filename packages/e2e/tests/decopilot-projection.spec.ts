 ...
 *    endpoint) that accepts a crafted chunk stream carrying a
...
test.describe("decopilot projection — hosted happy path", () => {
  test.skip(
    ...
    "Requires E2E_ANTHROPIC_KEY — the hosted path spawns a real harness via " +
      ...
      "publishes a canned chunk stream to JetStream so the hosted path can be " +
...
test.describe("decopilot projection — requires_action", () => {
  test.skip(
    ...
    "requires_action is produced when resolveThreadStatus maps finishReason=" +
      ...
      "test hook that injects a crafted chunk stream whose finish-step chunk carries " +
      ...
      "that seeds a run, publishes crafted chunks, and returns the final " +
