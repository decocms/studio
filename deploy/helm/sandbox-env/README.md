...
| `housekeeper.idleTtlConfigMap` | `""` | read the sweep's TTL from the sandbox controller's `sandbox-controller-idle-ttl` ConfigMap instead of `idleTtlSeconds`, once the controller takes claims |
